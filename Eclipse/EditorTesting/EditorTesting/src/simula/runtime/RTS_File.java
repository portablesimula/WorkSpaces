...
/// 
/// Link to GitHub: <a href="https://github.com/portablesimula/WorkSpaces/Eclipse/blob/main/SimulaCompiler2/Simula/src/simula/runtime/RTS_File.java"><b>Source File</b></a>.
/// 
...
//		System.out.println("RTS_File.trySelectFile: " + fileName);
		File file = new File(fileName);
		if (file.exists())
			return (file);
		if (!file.isAbsolute()) {
			File tryFile = new File(RTS_Option.RUNTIME_USER_DIR, fileName);
//			System.out.println("RTS_File.trySelectFile: 1) " + tryFile);
			if (tryFile.exists())
				return (tryFile);
			File dir = new File(System.getProperty("user.dir", null));
//			System.out.println("RTS_File.trySelectFile: 2) " + tryFile);
