...
public class RepairSTM_MethodVisitor extends MethodVisitor {
	...
	private void treat_STMMethod(String methodName) {
        if(Option.internal.TRACE_REPAIRING) Util.println("*** PASS 1 - FIND AND TREAT ALL LABELS IN "+methodName+"("+classFileName+") ***");
...
//				if(name.equals("_LABEL"))  {
        ...
        } else if(Option.verbose) Util.println("Repair_STMMethodVisitor: "+labelHandles.size()+" labels found in Method "+method);
        
        if(Option.internal.TRACE_REPAIRING) Util.println("*** PASS 2 - FIND AND TREAT _JUMPTABLE ***");
...
//			  default: Util.BREAK("Repair_STMMethodVisitor.treatLABEL: IMPOSSIBLE-2");
			...
			Util.IERR("More than 32767 labels in same Block (cst="+cst+')');
		} else {
