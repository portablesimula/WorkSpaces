...
// Simula-2.0 Compiled at Thu Feb 27 10:05:44 CET 2025
...
public boolean isQPSystemBlock() { return(true); }
    ...
    public simtst49 _STM() {
        ...
        // JavaLine 152 <== SourceLine 53
        {
            // BEGIN INSPECTION 
            _inspect_53_3=ra_1;
            if(_inspect_53_3 instanceof simtst49_A  _connID_4) { // WHEN simtst49_A DO 
                // JavaLine 157 <== SourceLine 55
                {
                   ...
                   ,new FOR_SingleElt<Number>(new RTS_NAME<Number>(){ public Number put(Number x_){_inspect_53_3.k=x_.intValue(); return(x_);};  public Number get(){return((Number)_inspect_53_3.k); }	},new RTS_NAME<Number>() { public Number get(){return(RTS_UTIL._IADD(_inspect_53_3.k,1)); }})
                        ...
                        l_1=_inspect_53_3.k; {
                            // JavaLine 164 <== SourceLine 56
                            RTS_BASICIO.sysout().outint(RTS_UTIL._IADD(_inspect_53_3.k,l_1),3);
                        }
                    }
                }
            }
            else if(_inspect_53_3 instanceof simtst49_B  _connID_5) { // WHEN simtst49_B DO 
                // JavaLine 171 <== SourceLine 57
                {
                    new SimulaTest_err((_CUR),new RTS_TXT("Wrong statement selected(3)."));
                }
            }
            else { // OTHERWISE 
                // JavaLine 177 <== SourceLine 58
                new SimulaTest_err((_CUR),new RTS_TXT("Wrong statement selected(4)."));
            } // END OTHERWISE 
        } // END INSPECTION
        ;
        // JavaLine 182 <== SourceLine 60
        t_1=RTS_ENVIRONMENT.copy(copy(RTS_TXT.strip(RTS_BASICIO.sysout().image)));
        ;
        // JavaLine 185 <== SourceLine 61
        RTS_TXT.setpos(RTS_BASICIO.sysout().image,1);
        ;
        // JavaLine 188 <== SourceLine 62
        RTS_UTIL._ASGTXT(RTS_BASICIO.sysout().image,null);
        ;
        // JavaLine 191 <== SourceLine 64
        if(_VALUE(RTS_UTIL._TXTREL_EQ(t_1,new RTS_TXT("  0  2")))) {
            ;
        } else {
            {
                // JavaLine 196 <== SourceLine 65
                new SimulaTest_err((_CUR),new RTS_TXT("for-loop(5)."));
                ;
                // JavaLine 199 <== SourceLine 66
                RTS_BASICIO.sysout().outtext(CONC(new RTS_TXT("            Erroneus values : "),t_1));
                ;
                RTS_BASICIO.sysout().outimage();
                ;
        ...
        ;
        // JavaLine 209 <== SourceLine 24
        if(_VALUE(noMessage)) {
            ;
        } else {
            {
                // JavaLine 214 <== SourceLine 25
                if(_VALUE(found_error)) {
                    {
                        // JavaLine 217 <== SourceLine 26
                ...
                ;
                // JavaLine 245 <== SourceLine 32
                if(_VALUE(false)) {
                    {
                        // JavaLine 248 <== SourceLine 33
    ...
    public static RTS_PROGINFO _INFO=new RTS_PROGINFO("simtst49.sim","10 simtst49",1,11,11,13,13,22,15,23,17,27,19,39,21,54,32,16,35,17,42,18,45,19,53,22,57,25,60,27,65,28,78,30,81,31,84,32,87,34,92,35,95,36,103,39,108,40,115,41,127,43,130,44,133,45,136,47,141,48,144,49,152,53,157,55,164,56,171,57,177,58,182,60,185,61,188,62,191,64,196,65,199,66,209,24,214,25,217,26,226,27,231,28,233,29,245,32,248,33,269,69);
} // End of Class
