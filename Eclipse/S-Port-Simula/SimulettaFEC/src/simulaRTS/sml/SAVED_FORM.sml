...
%    end;
 ...
 Visible routine ED_ATRDES;
 import ref(atrdes) ad;
 begin
%+M	   ED_STR("FORM.ED_ATRDES: ident="); ED_IDT(ad.ident); ED_OUT;
 end;
...
%    end;
 ...
 Visible routine XXX_B_FNP_ACC; --- DENNE MANGLET I 'RTSINT1.def'
 import name(infix(parqnt)) fnp;      --  Parameter's address
 exit label psc;
...
-- 	   infix(parqnt) par; par:=var(fnp); 
% +M	   ED_STR("FORM.B_FNP_ACC: par.fp="); ED_OADDR(par.fp); ED_OUT;
% +M	   ED_STR("FORM.B_FNP_ACC: par.ap="); ED_OADDR(par.ap); ED_OUT;
% +M	   ED_STR("FORM.B_FNP_ACC: par.ent="); ED_OADDR(par.ent); ED_OUT;
% +M	   ED_ATRDES(par.fp);
-- 	PRT2("MISSING ROUTINE 'B_FNP_ACC' IN 'RTSINIT'",getCallChain);
-- 	TERMIN(3,"ERROR: B_FNP_ACC --- DENNE MANGLET I 'RTSINT1.def' ");
 end;
 
 Visible routine XXX_B_FNP_ASS; --- DENNE MANGLET I 'RTSINT1.def'
 import name(infix(parqnt)) fnp;      --  Parameter's address
% begin
% 	TERMIN(3,"ERROR: B_FNP_ASS --- DENNE MANGLET I 'RTSINT1.def' ");
% Visible routine BnpAss;
% import name(infix(parqnt)) fnp;         --  Parameter's address
 exit label psc;
 begin curins.lsc:= psc;
