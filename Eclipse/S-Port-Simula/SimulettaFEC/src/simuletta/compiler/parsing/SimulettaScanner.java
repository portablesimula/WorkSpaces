...
//	public static Hashtable<String, Object> mnemonics;
	public static Hashtable<String, Mnemonic> mnemonics;
	 ...
	 */
	public SimulettaScanner(final String name,final Reader reader,final boolean editorMode) {
		this.sourceFileReader=new SourceFileReader(name,reader);
		this.editorMode=editorMode;
		Global.sourceLineNumber=1;
//		mnemonics=new Hashtable<String, Object>();
		mnemonics=new Hashtable<String, Mnemonic>();
		if(Option.SELECT!=null) this.setSelectors(Option.SELECT);
	}
...
//    //********************************************************************************
//    //*** Define literal mnemonic
//    //********************************************************************************
//	public static void define(String ident,Token value) {
//		mnemonics.put(ident.toUpperCase(),value);
//	}
//
//    //********************************************************************************
//    //*** Define Macro mnemonic
//    //********************************************************************************
//	public static void defineMacro(String ident,MacroDefinition value) {
//		Object old=mnemonics.get(ident);
//		if(old!=null) {
//			Util.BREAK("Mnemonic "+ident+" already defined: "+value);
//			Util.ERROR("Mnemonic "+ident+" already defined: "+value);
//		}
//		mnemonics.put(ident,value);
//	}

    //********************************************************************************
    //*** Define Macro mnemonic
    //********************************************************************************
	public static void defineMnemonic(String ident,Mnemonic value) {
		Object old=mnemonics.get(ident);
		if(old!=null) {
			Util.BREAK("Mnemonic "+ident+" already defined: "+value);
			Util.ERROR("Mnemonic "+ident+" already defined: "+value);
    ...
    private Token scanBasic() {
    	if(Option.TRACE_SCAN) Util.TRACE("SimulettaScanner.scanBasic, "+edcurrent());
    	while(true)	{
...
//    					if(val instanceof Token) return((Token)val);
    		...
    		switch(current) {
    		    case '=':	               return(newToken(KeyWord.EQ));
	            case '>':
	            ...
	            case ' ':
	            case ';':           /* Semicolon is white-space in Simuletta
	            case '\b':			/* BS */
	            case '\t':			/* HT */
	           	//case '\v':		/* VT */
	            case '\f':			/* FF */
...
//    	if(Parser.prevToken.getKeyWord()==KeyWord.IDENTIFIER) Util.ERROR("Misplaced identifier "+name+" directly after "+Parser.prevToken);
	...
	private Token scanIdentifier() {
	    ...
	    if(Option.TRACE_SCAN) Util.TRACE("Token.scanIdentifier: name=\""+name+"\"");
	    switch(name.toUpperCase().charAt(0)) {
	        case 'A':
		        ...
		        if(name.equalsIgnoreCase("ASSERT"))	    return(Token.ASSERT);
		        break;
	        case 'B':
	        ...
	        case 'R':
	        	...
	        	if(name.equalsIgnoreCase("RECORD"))     return(Token.RECORD);
...
//	        	if(name.equals("String"))    				return(Token.STRING); // Special Case
	        ...
	        case 'T':
	        	...
	        	break;
	        case 'V':
	        	if(name.equalsIgnoreCase("VAR"))	   return(Token.VAR);
	        	if(name.equalsIgnoreCase("VARIANT"))   return(Token.VARIANT);
...
//    		if(editorMode) tokenQueue.add(newToken(KeyWord.STRING));
	...
	private Token scanDirectiveLine() {
		...
		if(current==' ') {
			...
			if(lineSelected()) {
				...
				if(Option.TRACE_SELECTION) Util.println("SimulettaScanner.scanDirectiveLine: SELECTED: next="+next);
				if(next=='%') return(scanDirectiveLine());
				else pushBack(current);
		    } else {
		    	if(Option.TRACE_SELECTION) Util.println("SimulettaScanner.scanDirectiveLine: NOT SELECTED");
				readUntilEndofLine();
		    }
        	if(Option.TRACE_SELECTION) BREAK("SimulettaScanner.scanDirectiveLine: RETURN COMMENT");
		    return (newToken(KeyWord.COMMENT));
	...
	private boolean lineSelected() {
		while (true) {
			if (current == '+') {
				getNext();
				if(Option.TRACE_SELECTION) Util.println("SimulettaScanner.lineSelected: CHECK SELECTOR: +"+(char)current);
