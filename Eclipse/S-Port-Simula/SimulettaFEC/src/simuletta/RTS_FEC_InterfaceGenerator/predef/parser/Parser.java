...
public final class Parser {
	...
	public static InterfaceScanner simulettaScaner;

	// *********************************************************************************
	// *** Open
	// *********************************************************************************
	public static void open(final String name,final Reader reader) {
		simulettaScaner = new InterfaceScanner(name,reader,false);
		prevToken = null;
		currentToken = null;
	...
	public static boolean accept(final int ... s) {
		for(int i=0;i<s.length;i++) {
			if (Parser.currentToken.getKeyWord() == s[i]) {
				if(RTS_FEC_Interface_Option.TRACE_PARSE) Util.TRACE("Parser.accept("+KeyWord.ed(s[i])+") == True  "+currentToken);
				nextSymb();
				return (true);
			}
		}
...
//		}
	...
	public static boolean acceptAnyExcept(final int ... s) {
		for(int i=0;i<s.length;i++) {
			if (Parser.currentToken.getKeyWord() == s[i]) {
				if(RTS_FEC_Interface_Option.TRACE_PARSE) Util.TRACE("Parser.accept("+KeyWord.ed(s[i])+") == false  "+currentToken);
				return (false);
			}
		}
//		if (Option.TRACING) {
...
//		}
	...
	public static boolean checkNext(final int s) {
		if (Parser.currentToken.getKeyWord() == s) {
			if(RTS_FEC_Interface_Option.TRACE_PARSE) Util.TRACE("Parser.checkNext("+s+") == True  "+currentToken);
			return (true);
		}
...
//	}
	...
	public static void TRACE(final String msg) {
		if(RTS_FEC_Interface_Option.TRACE_PARSE) Util.TRACE(msg + ", current=" + Parser.currentToken + ", prev=" + Parser.prevToken);
	}
...
//			return;
//		}
		if(RTS_FEC_Interface_Option.TRACE_PARSE || RTS_FEC_Interface_Option.TRACE_MACRO_SCAN) println("Parser.nextSymb: "+Parser.currentToken+", prevToken="+Parser.prevToken);
	}
