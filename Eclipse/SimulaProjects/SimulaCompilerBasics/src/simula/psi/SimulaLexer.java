...
//    	IO.println("sourceText:        " + (""+sourceText).replace("\r", "\\r").replace("\n", "\\n"));
    ...
    public SimulaLexer(AstBuilder astBuilder) {
...
//    		IO.println("SimulaLexer.advance(from queue): nextLineNumber: " + nextLineNumber + ", tokenStartLine: " + tokenStartLine);
    		...
    		if (currentPosition >= textEndOffset) {
    			...
    			if(EOF != null) {
    				...
    				Util.syntaxError(astBuilder, prevParserToken, "Incomplete program: Attempt to advance Lexer beyond end of source text");
    			}
    			EOF = prevParserToken;
    			// return;
    		} else {
...
//        return new Identifier(tokenStartLine, sourceText, tokenStartOffset, currentPosition);
    ...
    private LexToken scanNumber() {
        ...
        try {
            ...
            Util.syntaxError(astBuilder, token, "Integer number out of range: " + token.edText());
            return token;
        }
    ...
    private LexToken scanDotDigit(StringBuilder number) {
        ...
        try {
            ...
            Util.syntaxError(astBuilder, token, "Illegal number: " + token.edText());
            return token;
        }
    }
    ...
    private LexToken scanCharacterConstant() {
        ...
        if(err != null) Util.syntaxError(astBuilder, token, err);
        return token;
    }
