...
//			for(ExternalDeclaration xdecl:program.externalHead) {
	...
	public static Type readAttributeFile(final AstBuilder astBuilder,final String identifier, final File file, final BlockDeclaration enclosure) {
		...
		try {
			...
			BlockDeclaration module = AttributeFileIO.readPrecompiled(astBuilder, file.toString(),bytes);
	...
	private static BlockDeclaration readPrecompiled(AstBuilder astBuilder, String fileID,byte[] attrFile) throws IOException {
...
//			public void readExternalAttributeFile() {
				...
				if (jarFile == null) {
					Util.syntaxError(astBuilder, "Can't find attribute file: " + xdecl.identifier + '[' + xdecl.externalIdent + ']');
				} else {
					if(checkJarFiles(jarFile)) {
						BlockDeclaration enclosure = StandardClass.BASICIO;
						AttributeFileIO.readAttributeFile(astBuilder, xdecl.identifier, jarFile, enclosure);
					}
				}		
//			}
