...
//	}
	...
	/// Print a error message.
	/// @param msg the message
	public static void syntaxError(final AstBuilder astBuilder, final String msg) {
		syntaxError(astBuilder, astBuilder.prevParserToken(), msg);
	}
	
	public static void syntaxError(final AstBuilder astBuilder, final LexToken token, final String msg) {
...
//        Position end = new Position(error.getLine() - 1, error.getCharPosition() + error.getLength());
        ...
        // LSP bruker 0-indekserte linjer og tegn
        SimPosition start = new SimPosition(token.lineNumber - 1, token.startOffset);
        SimPosition end = new SimPosition(token.lineNumber - 1, token.endOffset);
		SimulaDiagnostic diagnostic = new SimulaDiagnostic(SimulaDiagnostic.Severity.Error, new SimRange(start, end), mss);
		
		astBuilder.addDiagnostic(diagnostic);
	}
...
//		IO.println("\n\nUtil.semanticError: ADD ERROR TEXT: " + elt + " " + msg);
        ...
        // LSP bruker 0-indekserte linjer og tegn
        SimPosition start = new SimPosition(first.lineNumber - 1, first.startOffset);
        SimPosition end = new SimPosition(last.lineNumber - 1, last.endOffset);
		SimulaDiagnostic diagnostic = new SimulaDiagnostic(SimulaDiagnostic.Severity.Error, new SimRange(start, end), mss);
		elt.astBuilder.addDiagnostic(diagnostic);
		Global.sourceLineNumber = lno;
//		Thread.dumpStack();
	}
...
//		else
	...
	public static int IPOW(final AstBuilder astBuilder, final long base, long x) {
		if (x == 0) {
			if (base == 0)
				syntaxError(astBuilder, "Exponentiation: " + base + " ** " + x + "  Result is undefined.");
			return (1); // any ** 0 ==> 1
		} else if (x < 0)
			syntaxError(astBuilder, "Exponentiation: " + base + " ** " + x + "  Result is undefined.");
		else if (base == 0)
		...
		if(res > Integer.MAX_VALUE || res < Integer.MIN_VALUE)
			syntaxError(astBuilder, "Arithmetic overflow: "+base+" ** "+x+" ==> "+res
