...
public final class LocalObject extends Expression {
	...
	private LocalObject(final AstBuilder astBuilder, final String ident) {
		super(astBuilder);
	...
	static Expression expectThisIdentifier(AstBuilder astBuilder) {
		if (Option.internal.TRACE_PARSE)
			Util.TRACE("Parse ThisObjectExpression, current=" + PsiParse.currentLexToken(astBuilder));
		String classIdentifier = PsiParse.expectIdentifier(astBuilder).edText();
		Expression expr = new LocalObject(astBuilder, classIdentifier);
		return(expr);
	}
