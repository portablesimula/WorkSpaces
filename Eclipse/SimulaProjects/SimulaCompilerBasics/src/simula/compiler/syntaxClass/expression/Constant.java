...
public final class Constant extends Expression {
	...
	public Constant(final AstBuilder astBuilder, final Type type,final Object value) {
		super(astBuilder);
		this.type=type;
		this.value = value;
	}
...
//    }
    static Constant createRealType(final AstBuilder astBuilder, final float value) {
    	Type type=Type.Real;
    	return(new Constant(astBuilder, type,value));
    }
	
	/// Create a long real type Constant.
	/// @param value a long real type value
	/// @return the resulting Constant
    static Constant createLongRealType(final AstBuilder astBuilder, final double value) {
    ...
    /// @return the resulting Constant
    static Constant evaluate(final AstBuilder astBuilder, final int opr,final Number rhn) { 
		...
		return(new Constant(astBuilder, type,result));
    }
    ...
    /// @param rhn a right hand Number
    /// @return the resulting Constant
    static Constant evaluate(final AstBuilder astBuilder, final Number lhn,final int opr,final Number rhn) { 
		...
		switch(type.keyWord) {
			case Type.T_INTEGER -> {
				switch(opr) {
	        		...
	        		case KeyWord.EXP    -> result=Util.IPOW(astBuilder, lhn.longValue(),rhn.longValue());
	        		default     -> Util.IERR();
				}
				if(result.longValue() > Integer.MAX_VALUE || result.longValue() < Integer.MIN_VALUE)
					Util.syntaxError(astBuilder, "Arithmetic overflow: "+lhn+' '+KeyWord.edit(opr)+' '+rhn);
