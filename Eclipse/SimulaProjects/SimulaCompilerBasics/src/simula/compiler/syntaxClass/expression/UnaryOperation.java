...
public final class UnaryOperation extends Expression {
	...
	private UnaryOperation(final AstBuilder astBuilder, final int oprator,final Expression operand) {
		super(astBuilder);
		this.oprator = oprator; this.operand = operand;
//		astBuilder.startSubtree(PsiTree.Kind.unaryOperation, "UnaryOperation");
		if(this.operand==null)
		{ Util.syntaxError(astBuilder, "Missing operand after unary "+KeyWord.edit(oprator));
		  this.operand=new MissingExpression(astBuilder);
		}
		this.operand.backLink=this;
//		astBuilder.doneSubtree(PsiTree.Kind.unaryOperation, this);
	}
	...
	static Expression create(final AstBuilder astBuilder, final int oprator,final Expression operand) {
		if (oprator == KeyWord.PLUS || oprator == KeyWord.MINUS) {
			try { // Try to Compile-time Evaluate this expression
				...
				if(rhn!=null) {
					return(Constant.evaluate(astBuilder, oprator, rhn));
				}  
			} catch(Exception e) {}
		}
		return(new UnaryOperation(astBuilder, oprator, operand));
	}
