...
public final class ObjectGenerator extends Expression {
	...
	private ObjectGenerator(final AstBuilder astBuilder, final String ident,final Vector<Expression> params) {
		super(astBuilder);
	...
	static Expression expectNew(AstBuilder astBuilder) {
		if (Option.internal.TRACE_PARSE)
			Util.TRACE("Parse ObjectGenerator, current=" + PsiParse.currentLexToken(astBuilder));
		...
		if (PsiParse.accept(astBuilder, KeyWord.BEGPAR)) {
			do {
				...
				if(par==null) Util.syntaxError(astBuilder, "Missing class parameter");
				else params.add(par);
			} while (PsiParse.accept(astBuilder, KeyWord.COMMA));
			PsiParse.expect(astBuilder, KeyWord.ENDPAR);
		}

		Expression expr = new ObjectGenerator(astBuilder, classIdentifier, params);
		return (expr);
	}
