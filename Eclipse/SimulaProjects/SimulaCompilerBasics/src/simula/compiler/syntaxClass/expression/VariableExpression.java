...
public final class VariableExpression extends Expression {
	...
	public VariableExpression(final AstBuilder astBuilder, final String identifier) {
		super(astBuilder);
	...
	public static VariableExpression expectVariable(final AstBuilder astBuilder, final String ident) {
		if (Option.internal.TRACE_PARSE)
			Util.TRACE("Parse Variable, current=" + PsiParse.currentLexToken(astBuilder));
		IO.println("Parse Variable, current=" + PsiParse.currentLexToken(astBuilder));
		Util.STOP();
		VariableExpression variable = new VariableExpression(astBuilder, ident);
...
//				IO.println("VariableExpression.expectVariable: GOT BEGPAR OR COMMA");
				Expression par = acceptExpression(astBuilder);
				if (par == null)
					Util.syntaxError(astBuilder, "Missing procedure parameter");
				else{
...
//			case ObjectKind.Switch:
	...
	private String editVariable(final String rightPart) {
		...
		switch (declaredAs.declarationKind) {
			...
			case ObjectKind.Class:
			case ObjectKind.StandardClass:
				Util.codingError("Illegal use of class identifier: " + declaredAs.identifier);
			...
			case ObjectKind.Parameter:
				...
				case Parameter.Kind.Procedure: // Parameter Procedure
					...
					if (par.mode == Parameter.Mode.value)
						Util.codingError("Parameter " + this + " by Value is not allowed - Rewrite Program");
