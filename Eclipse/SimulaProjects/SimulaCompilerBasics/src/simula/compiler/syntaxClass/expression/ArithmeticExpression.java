...
public final class ArithmeticExpression extends Expression {
	...
	private ArithmeticExpression(final AstBuilder astBuilder, final Expression lhs, final int opr, final Expression rhs) {
		super(astBuilder);
		this.opr = opr;
//		astBuilder.startSubtree(PsiTree.Kind.arithmeticExpression, "ArithmeticExpression");
		if (lhs == null) {
			Util.syntaxError(astBuilder, "Missing operand before " + KeyWord.edit(opr));
			this.lhs = new MissingExpression(astBuilder);
		} else
			this.lhs = lhs;
		if (rhs == null) {
			Util.syntaxError(astBuilder, "Missing operand after " + KeyWord.edit(opr));
			this.rhs = new MissingExpression(astBuilder);
		} else
			this.rhs = rhs;
		this.lhs.backLink = this.rhs.backLink = this;
//		astBuilder.doneSubtree(PsiTree.Kind.arithmeticExpression, this);
	}
	...
	static Expression create(final AstBuilder astBuilder, final Expression lhs, final int opr, final Expression rhs) {
		try { // Try to Compile-time Evaluate this expression
			Number lhn = lhs.getNumber();
			if (lhn != null) {
				Number rhn = rhs.getNumber();
				if (rhn != null)
					return (Constant.evaluate(astBuilder, lhn, opr, rhn));
			}
		} catch (Exception e) {
			Util.syntaxError(astBuilder, "Arithmetic overflow: " + lhs + ' ' + KeyWord.edit(opr) + ' ' + rhs + "   " + e);
			e.printStackTrace();
		}
		return (new ArithmeticExpression(astBuilder, lhs, opr, rhs));
	}

//	@Override
//	public Expression evaluate(final PsiBuilder astBuilder) {
