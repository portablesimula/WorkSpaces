...
public final class SwitchDeclaration extends ProcedureDeclaration {
	...
	public SwitchDeclaration(AstBuilder astBuilder, final String ident) {
		super(astBuilder, ident, ObjectKind.Procedure);
		...
		PsiParse.expect(astBuilder, KeyWord.ASSIGNVALUE);
		do { switchList.add(Expression.expectExpression(astBuilder, "designational"));
		} while (PsiParse.accept(astBuilder, KeyWord.COMMA));
		if (Option.internal.TRACE_PARSE)
			PsiParse.TRACE("Parse SwitchDeclaration(3), switchList=" + switchList);
		new Parameter(astBuilder, "_SW", Type.Integer, Parameter.Kind.Simple).into(parameterList);
