...
public final class Parameter extends Declaration {
	...
	public Parameter(final AstBuilder astBuilder, final String identifier) {
	...
	Parameter(final AstBuilder astBuilder, final String identifier, final Type type, final int kind) {
		this(astBuilder, identifier);
		this.type = type;
		this.kind = kind;
	}
	...
	public Parameter(final AstBuilder astBuilder, final String identifier, final Type type, final int kind, final int nDim) {
		this(astBuilder, identifier, type, kind);
		this.nDim = nDim;
	}
	...
	public void into(final Vector<Parameter> parameterList) {
		for (Parameter par : parameterList)
			if (Util.equals(par.identifier, this.identifier)) {
				Util.syntaxError(astBuilder, "Parameter already defined: " + identifier);
				return;
			}
	...
	void setMode(final int mode) {
		if (this.mode != 0)
			Util.syntaxError(astBuilder, "Parameter " + identifier + " is already specified by " + this.mode);
		this.mode = mode;
	}
