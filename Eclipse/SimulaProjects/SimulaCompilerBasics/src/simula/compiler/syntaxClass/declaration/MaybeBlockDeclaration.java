...
public final class MaybeBlockDeclaration extends BlockDeclaration {
	...
	public MaybeBlockDeclaration(final AstBuilder astBuilder, final String identifier) {
		super(astBuilder, identifier);
		if(identifier != null)
			modifyIdentifier(identifier);
//		else modifyIdentifier("Block" + firstLineNumber());
		else modifyIdentifier("Block" + astBuilder.getSourceLineNumber());
	}
...
//	}
	...
	public BlockStatement expectMaybeBlock(AstBuilder astBuilder) {
		...
		astBuilder.consume(KeyWord.BEGIN); // consume BEGIN (add it to 'current tree')

		parseBlock(astBuilder);
...
//		IO.println("MaybeBlockDeclaration.expectMaybeBlock: GOT END or EOF");
		...
		BlockStatement block = new BlockStatement(astBuilder, this, debugName);
		if(Option.TRACE_ACCEPT_STATEMENT > 0) IO.println("MaybeBlockDeclaration.expectMaybeBlock: ENDOF " + debugName + "  " + block);
		Global.setScope(declaredIn);
		if (Option.internal.TRACE_PARSE)
			Util.TRACE("Line " + astBuilder.getSourceLineNumber() + ": DONE "+debugName+" started at line: " + lno + ": " + this);
		return (block);
	}
