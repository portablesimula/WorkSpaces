...
//	public int lastLineNumber;
	...
	protected BlockDeclaration(final AstBuilder astBuilder, String identifier) {
	...
	private BlockDeclaration(final AstBuilder astBuilder, final String identifier,final int declarationKind) {
		super(astBuilder, identifier);
	...
	protected static void expectFormalParameterPart(final AstBuilder astBuilder, final Vector<Parameter> pList) {
		do { // ParameterPart = Parameter ; { Parameter ; }
			new Parameter(astBuilder, PsiParse.expectIdentifier(astBuilder).edText()).into(pList);
	...
	// *** Parsing: parseBlock
	// ***********************************************************************************************
	protected void parseBlock(final AstBuilder astBuilder) {
		...
		astBuilder.startSubtree(PsiTree.Kind.declaration, "May be first Declaration");
		DeclarationList declarationList=this.declarationList;
		Vector<SyntaxElement> declarations = null;
		while( (declarations = Declaration.acceptDeclaration(astBuilder)) != null) {
			...
			astBuilder.doneSubtree(PsiTree.Kind.declaration, declarations);
			astBuilder.startSubtree(PsiTree.Kind.declaration, "May be NextDeclaration");
		}
		astBuilder.dropSubtree(PsiTree.Kind.declaration, " is not a Declaration: DROP LAST MAYBE DECLARATION TREE");
		
		LOOP:while(true) {
			LexToken token = PsiParse.acceptParserToken(astBuilder, KeyWord.END, KeyWord.EOF);
//	        IO.println("BlockDeclaration.parseBlock: TESTING token: " + token);
	        if(token != null) {
		        if(token.keyWord == KeyWord.EOF)
		        	Util.syntaxError(astBuilder, token, "Missing final block end");
	        	break LOOP;
	        }
