...
public class ClassDeclaration extends BlockDeclaration {
	...
	protected ClassDeclaration(final AstBuilder astBuilder, String identifier) {
		super(astBuilder, identifier);
	...
	public static ClassDeclaration expectClassDeclaration(final AstBuilder astBuilder, final String ident) {
		ClassDeclaration cls = new ClassDeclaration(astBuilder, null);
		cls.sourceFileName = Global.sourceFileName;
		cls.prefix = ident;
		cls.declaredIn.hasLocalClasses = true;
		if (cls.prefix == null)
			cls.prefix = StandardClass.CLASS.identifier;
//		cls.modifyIdentifier(PsiParse.expectIdentifier(astBuilder, PsiTextPanel.styleNameClassIdent).edText());
		cls.modifyIdentifier(PsiParse.expectIdentifier(astBuilder, "PsiTextPanel.styleNameClassIdent").edText());
		if (PsiParse.accept(astBuilder, KeyWord.BEGPAR)) {
			expectFormalParameterPart(astBuilder, cls.parameterList);
			PsiParse.expect(astBuilder, KeyWord.SEMICOLON);
			acceptValuePart(astBuilder, cls.parameterList);
			acceptParameterSpecificationPart(astBuilder, cls.parameterList);
		} else
			PsiParse.expect(astBuilder, KeyWord.SEMICOLON);

		acceptProtectionPart(astBuilder, cls);
		if (PsiParse.accept(astBuilder, KeyWord.VIRTUAL))
			VirtualSpecification.expectVirtualPart(astBuilder, cls);
		expectClassBody(astBuilder, cls);
	...
	private static void acceptValuePart(final AstBuilder astBuilder, final Vector<Parameter> pList) {
		if (PsiParse.accept(astBuilder, KeyWord.VALUE)) {
			do {
				String identifier = PsiParse.expectIdentifier(astBuilder).edText();
				...
				if (parameter == null) {
					Util.syntaxError(astBuilder, "Identifier " + identifier + " is not defined in this scope");
					parameter = new Parameter(astBuilder, identifier);
				}
				parameter.setMode(Parameter.Mode.value);
			} while (PsiParse.accept(astBuilder, KeyWord.COMMA));
			PsiParse.expect(astBuilder, KeyWord.SEMICOLON);
		}
	}
	...
	private static void acceptParameterSpecificationPart(final AstBuilder astBuilder, final Vector<Parameter> pList) {
		if (Option.internal.TRACE_PARSE)
			PsiParse.TRACE("Parse ParameterSpecifications");
		while (true) {
			Type type;
			int kind = Parameter.Kind.Simple;
			type = PsiParse.acceptType(astBuilder);
			if (PsiParse.accept(astBuilder, KeyWord.ARRAY)) {
				if (type == null) {
	...
	private static void acceptProtectionPart(final AstBuilder astBuilder, ClassDeclaration cls) {
		while (true) {
			if (PsiParse.accept(astBuilder, KeyWord.HIDDEN)) {
				if (PsiParse.accept(astBuilder, KeyWord.PROTECTED))
					expectHiddenProtectedList(astBuilder, cls, true, true);
				else
					expectHiddenProtectedList(astBuilder, cls, true, false);
			} else if (PsiParse.accept(astBuilder, KeyWord.PROTECTED)) {
				if (PsiParse.accept(astBuilder, KeyWord.HIDDEN))
					expectHiddenProtectedList(astBuilder, cls, true, true);
				else
					expectHiddenProtectedList(astBuilder, cls, false, true);
			} else
				break;
		}	
	}
	...
	private static void expectHiddenProtectedList(final AstBuilder astBuilder, final ClassDeclaration cls, final boolean hidden, final boolean prtected) {
		do {
			String identifier = PsiParse.expectIdentifier(astBuilder).edText();
			if (hidden)
				cls.hiddenList.add(new HiddenSpecification(astBuilder, cls, identifier));
			if (prtected)
				cls.protectedList.add(new ProtectedSpecification(astBuilder, cls, identifier));
		} while (PsiParse.accept(astBuilder, KeyWord.COMMA));
		PsiParse.expect(astBuilder, KeyWord.SEMICOLON);
	}
	...
	/// </pre>
	/// @param cls the ClassDeclaration
	private static void expectClassBody(final AstBuilder astBuilder, ClassDeclaration cls) {
		if (PsiParse.accept(astBuilder, KeyWord.BEGIN)) {
			if (Option.internal.TRACE_PARSE)
				PsiParse.TRACE("Parse Block");
			
			cls.parseBlock(astBuilder);
			if(! cls.hasInner()) {
//				IO.println("ClassDeclaration.expectClassBody: ADD IMPLICIT INNER");
				cls.statements.add(new InnerStatement(astBuilder, false)); // Implicit INNER
			}
		} else {
			if(PsiParse.getCurrentParserToken(astBuilder).keyWord != KeyWord.SEMICOLON)
				cls.statements.add(Statement.acceptStatement(astBuilder));
			if(! cls.hasInner())
				cls.statements.add(new InnerStatement(astBuilder, false)); // Implicit INNER
		}
	}
