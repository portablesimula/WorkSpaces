...
public class SimpleVariableDeclaration extends Declaration {
	...
	public SimpleVariableDeclaration(final AstBuilder astBuilder, final Type type, final String identifier) {
	...
	SimpleVariableDeclaration(final AstBuilder astBuilder, final Type type, final String identifier, final boolean constant, final Constant constantElement) {
		this(astBuilder, type, identifier);
	...
	static Vector<SyntaxElement> expectSimpleVariable(final AstBuilder astBuilder, final Type type) {
		// identifier-list = identifier { , identifier }
		Vector<SyntaxElement> declarations = new Vector<SyntaxElement>();
		if (Option.internal.TRACE_PARSE)
			PsiParse.TRACE("Parse IdentifierList");
		do {
			String ident = PsiParse.expectIdentifier(astBuilder).edText();
			SimpleVariableDeclaration typeDeclaration = new SimpleVariableDeclaration(astBuilder, type, ident);
			if (PsiParse.accept(astBuilder, KeyWord.EQ))
				typeDeclaration.constantElement = Expression.expectExpression(astBuilder, "constant");
