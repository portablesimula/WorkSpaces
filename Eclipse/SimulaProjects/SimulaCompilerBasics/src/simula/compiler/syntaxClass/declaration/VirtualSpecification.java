...
public final class VirtualSpecification extends Declaration {
	...
	VirtualSpecification(final AstBuilder astBuilder, final String identifier, final Type type, final int kind, final int prefixLevel, final ProcedureSpecification procedureSpec) {
	...
	static void expectVirtualPart(final AstBuilder astBuilder, final ClassDeclaration cls) {
		PsiParse.expect(astBuilder, KeyWord.COLON);
		LOOP: while (true) {
//			astBuilder.startSubtree(PsiTree.Kind.virtualSpecification, "May be Virtual");
			Type type;
			if (PsiParse.accept(astBuilder, KeyWord.SWITCH)) {
				expectIdentifierList(astBuilder, cls, Type.Label, Kind.Switch);
			} else if (PsiParse.accept(astBuilder, KeyWord.LABEL)) {
				expectIdentifierList(astBuilder, cls, Type.Label, Kind.Label);
			} else {
				type = PsiParse.acceptType(astBuilder);
//				IO.println("VirtualSpecification.expectVirtualPart: LEXER TOKEN: " + astBuilder.getCurrentLexerToken());
				if (!PsiParse.accept(astBuilder, KeyWord.PROCEDURE)) {
//					astBuilder.dropSubtree(PsiTree.Kind.virtualSpecification, " is not a virtualSpecification: DROP LAST MAYBE TREE");
					break LOOP;
				}
...
//				IO.println("\n\nVirtualSpecification.expectVirtualPart: " + identifier);
				...
				if (PsiParse.accept(astBuilder, KeyWord.IS)) {
					if(type != null) Util.syntaxError(astBuilder, "An IS-specified virtual procedure can have its type only after IS.");
					type = PsiParse.acceptType(astBuilder);
					PsiParse.expect(astBuilder, KeyWord.PROCEDURE);
					procedureSpec = ProcedureSpecification.expectProcedureSpecification(astBuilder, type);						
					cls.virtualSpecList
							.add(new VirtualSpecification(astBuilder, identifier, type, Kind.Procedure, cls.prefixLevel(), procedureSpec));
				} else {
					cls.virtualSpecList.add(new VirtualSpecification(astBuilder, identifier, type, Kind.Procedure, cls.prefixLevel(), null));
					if (PsiParse.accept(astBuilder, KeyWord.COMMA))
						expectIdentifierList(astBuilder, cls, type, Kind.Procedure);
					else
						PsiParse.expect(astBuilder, KeyWord.SEMICOLON);
				}
//				IO.println("\n\nVirtualSpecification.expectVirtualPart: AFTER: " + identifier);
			}
//			astBuilder.doneSubtree(PsiTree.Kind.virtualSpecification, cls.virtualSpecList.lastElement());
//			astBuilder.startSubtree(PsiTree.Kind.virtualSpecification, "May be next Virtual");

		}
		if(cls.virtualSpecList.size()==0) Util.syntaxError(astBuilder, "Missing virtual specifier after VIRTUAL:");
	}
	...
	private static void expectIdentifierList(final AstBuilder astBuilder, final ClassDeclaration cls, final Type type, final int kind) {
		do {
			String identifier = PsiParse.expectIdentifier(astBuilder).edText();
			cls.virtualSpecList.add(new VirtualSpecification(astBuilder, identifier, type, kind, cls.prefixLevel(), null));
