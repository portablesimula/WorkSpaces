...
public class ProcedureDeclaration extends BlockDeclaration {
	...
	protected ProcedureDeclaration(final AstBuilder astBuilder, final String identifier,final int declarationKind) {
	...
	public static ProcedureDeclaration expectProcedureDeclaration(final AstBuilder astBuilder, final Type type) {
		ProcedureDeclaration proc = new ProcedureDeclaration(astBuilder, null, ObjectKind.Procedure);
		proc.sourceFileName = Global.sourceFileName;
//		proc.OLD_lineNumber = astBuilder.getSourceLineNumber();
		proc.type = type;
		if (Option.internal.TRACE_PARSE)
			PsiParse.TRACE("Parse ProcedureDeclaration, type=" + type);
//		proc.modifyIdentifier(PsiParse.expectIdentifier(astBuilder, PsiTextPanel.styleNameProcedure).edText());
		proc.modifyIdentifier(PsiParse.expectIdentifier(astBuilder, "PsiTextPanel.styleNameProcedure").edText());
//		IO.println("ProcedureDeclaration.expectProcedureDeclaration: "+proc.identifier);
		if (PsiParse.accept(astBuilder, KeyWord.BEGPAR)) {
			expectFormalParameterPart(astBuilder, proc.parameterList);
			PsiParse.expect(astBuilder, KeyWord.SEMICOLON);
			while(acceptModePart(astBuilder, proc.parameterList));
			expectSpecificationPart(astBuilder, proc);
		} else PsiParse.expect(astBuilder, KeyWord.SEMICOLON);
//		IO.println("ProcedureDeclaration.expectProcedureDeclaration: BEFORE PARSE PROCEDURE BLOCK");
		expectProcedureBody(astBuilder, proc);
...
//		proc.lastLineNumber = Global.sourceLineNumber;
	...
	private static boolean acceptModePart(AstBuilder astBuilder, Vector<Parameter> pList) {
		LexToken prevToken = PsiParse.getCurrentParserToken(astBuilder);
		if (PsiParse.accept(astBuilder, KeyWord.VALUE, KeyWord.NAME)) {
...
//			IO.println("ProcedureDeclaration.acceptModePart: END VALUE/NAME - Expect SEMICOLON");
	...
	private static void expectSpecificationPart(AstBuilder astBuilder, ProcedureDeclaration proc) {
		if (Option.internal.TRACE_PARSE)
		...
		LOOP: while(true) {
			...
			if (PsiParse.accept(astBuilder, KeyWord.SWITCH)) {
				...
				else if (PsiParse.accept(astBuilder, KeyWord.PROCEDURE)) kind = Parameter.Kind.Procedure;
...
//			IO.println("ProcedureDeclaration.expectSpecificationPart: END TYPE Identifiers - Expect SEMICOLON");
		...
		for (Parameter par : proc.parameterList) {
			if (par.kind != 0)
				...
				default:
					if (par.type == null)
						Util.syntaxError(astBuilder, "Missing specification of parameter: " + par.identifier);
				}
		}
	}
	...
	private static void expectProcedureBody(AstBuilder astBuilder, ProcedureDeclaration proc) {
...
//			IO.println("ProcedureDeclaration.expectProcedureBody: START PARSE PROCEDURE BLOCK");
			...
			proc.parseBlock(astBuilder);

			if (PsiParse.prevToken(astBuilder).keyWord == KeyWord.EOF) {
				Util.syntaxError(astBuilder, "Illegal termination of procedure declaration. Missing END.");
			}
		}
		else {
//			IO.println("ProcedureDeclaration.expectProcedureBody: START PARSE PROCEDURE BODY STATEMENT");
			LexToken token =astBuilder.getCurrentParserToken();
//			IO.println("ProcedureDeclaration.expectProcedureBody: token: " + token);
			if(token.keyWord != KeyWord.SEMICOLON) {
				Statement stm = Statement.acceptStatement(astBuilder);
	...
	public void doChecking() {
		...
		Global.enterScope(this);
			...
			if (virtualSpec != null) {
				...
				myVirtual = new VirtualMatch(astBuilder, virtualSpec, this);
