...
public final class ArrayDeclaration extends Declaration {
	...
	private ArrayDeclaration(final AstBuilder astBuilder, final String identifier, final Type type, final Vector<BoundPair> boundPairList) {
	...
	static Vector<SyntaxElement> expectArrayDeclaration(final AstBuilder astBuilder, final Type type) {
		if (Option.internal.TRACE_PARSE)
			Util.TRACE("Parse ArrayDeclaration, type=" + type + ", current=" + PsiParse.getCurrentParserToken(astBuilder));
		// IdentifierList = Identifier { , Identifier }
