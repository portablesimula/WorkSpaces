...
public final class ExternalDeclaration extends Declaration {
	...
	private ExternalDeclaration(final AstBuilder astBuilder, String identifier,String extIdentitier) {
	...
	public static Vector<SyntaxElement> expectExternalDeclaration(final AstBuilder astBuilder) {
		LexToken kind = PsiParse.acceptIdentifier(astBuilder);
		if (kind != null)
			Util.syntaxError(astBuilder, "*** NOT IMPLEMENTED: " + "External " + kind + " Procedure");
		Type expectedType = PsiParse.acceptType(astBuilder);
//		if (!(PsiParse.accept(astBuilder, KeyWord.CLASS) || PsiParse.accept(astBuilder, KeyWord.PROCEDURE)))
		if (!(PsiParse.accept(astBuilder, KeyWord.CLASS, KeyWord.PROCEDURE)))
//			Util.syntaxError(astBuilder.getCurrentLexerToken(), "parseExternalDeclaration: Expecting CLASS or PROCEDURE");
			Util.syntaxError(astBuilder, "parseExternalDeclaration: Expecting CLASS or PROCEDURE");
...
//			IO.println("ExternalDeclaration.expectExternalDeclaration: identifier=" + identifier);
			...
			if (PsiParse.accept(astBuilder, KeyWord.EQ)) {
...
//				IO.println("ExternalDeclaration.expectExternalDeclaration: " + token.getClass().getSimpleName());
				if(token instanceof SimpleString xident) {
					...
					Util.syntaxError(astBuilder, token, "Expecting external identifier string");
				}
				astBuilder.advanceLexer();
			}
			ExternalDeclaration externalDeclaration = new ExternalDeclaration(astBuilder, identifier, externalIdentifier);
...
//			IO.println("ExternalDeclaration.expectExternalDeclaration: externalDeclaration" + externalDeclaration);
			...
			if(jarFile == null) {
				Util.syntaxError(astBuilder, "Can't find attribute file: " + identifier + '[' + externalIdentifier + ']');
			}
			if (jarFile != null) {
				if(AttributeFileIO.checkJarFiles(jarFile)) {
					DeclarationScope scope = Global.getCurrentScope();
					Type moduleType = AttributeFileIO.readAttributeFile(astBuilder, identifier, jarFile, scope.getEnclosingBlock());
					if(moduleType == null) {
						if (expectedType != null) Util.syntaxError(astBuilder, "Missing external type: "+expectedType);
					} else if(expectedType == null) {
						// NOTHING
					} else if (!moduleType.equals(expectedType)) {
						if (expectedType != null)
							Util.syntaxError(astBuilder, "Wrong external type: "+moduleType+". Expected type: "+expectedType);
					}
				}
			}

			if (PsiParse.accept(astBuilder, KeyWord.IS)) {
				Util.syntaxError(astBuilder, "*** NOT IMPLEMENTED: " + "External non-Simula Procedure");
				break LOOP;
			}
			if (!PsiParse.accept(astBuilder, KeyWord.COMMA))
				break LOOP;
			identifier = PsiParse.expectIdentifier(astBuilder).edText();
		}
