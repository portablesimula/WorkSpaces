...
public abstract class Declaration extends SyntaxElement {
	...
	protected Declaration(final AstBuilder astBuilder, final String identifier) {
		super(astBuilder);
	...
	protected void checkAlreadyDefined() {
		...
		if (error)
			Util.syntaxError(astBuilder, identifier + " is alrerady defined in " + declaredIn.identifier);
		else if (warning)
...
//	protected static boolean acceptDeclaration(final PsiBuilder astBuilder, final BlockDeclaration enclosure) {
	protected static Vector<SyntaxElement> acceptDeclaration(final AstBuilder astBuilder) {
		if (Option.internal.TRACE_PARSE)
			PsiParse.TRACE("Parse Declaration");
		Declaration decl = null;
		LexToken maybePrefix = PsiParse.acceptIdentifier(astBuilder);
		if (maybePrefix != null) {
			if (PsiParse.accept(astBuilder, KeyWord.CLASS)) {
				decl = ClassDeclaration.expectClassDeclaration(astBuilder, maybePrefix.edText());
			} else {
				return null;
			}
		} else if (PsiParse.accept(astBuilder, KeyWord.ARRAY))
			return ArrayDeclaration.expectArrayDeclaration(astBuilder, Type.Real); // Default type real for arrays
		else if (PsiParse.accept(astBuilder, KeyWord.PROCEDURE))
			decl = ProcedureDeclaration.expectProcedureDeclaration(astBuilder, null);
		else if (PsiParse.accept(astBuilder, KeyWord.PRIOR)) {
			...
			decl = ProcedureDeclaration.expectProcedureDeclaration(astBuilder, type);
		} else if (PsiParse.accept(astBuilder, KeyWord.CLASS))
			decl = ClassDeclaration.expectClassDeclaration(astBuilder, null);
		else if (PsiParse.accept(astBuilder, KeyWord.SWITCH)) {
			LexToken ident = PsiParse.acceptIdentifier(astBuilder);
			if (ident == null) {
				// Switch Statement
				return null;
			}
			decl = new SwitchDeclaration(astBuilder, ident.edText());
		} else if (PsiParse.accept(astBuilder, KeyWord.EXTERNAL)) {
			Vector<SyntaxElement> ext = ExternalDeclaration.expectExternalDeclaration(astBuilder);	
			return ext;
		} else {
			Type type = PsiParse.acceptType(astBuilder);
			...
			if (PsiParse.accept(astBuilder, KeyWord.PROCEDURE)) {
				...
				return ArrayDeclaration.expectArrayDeclaration(astBuilder, type);
			}
			else 
				return SimpleVariableDeclaration.expectSimpleVariable(astBuilder, type);
