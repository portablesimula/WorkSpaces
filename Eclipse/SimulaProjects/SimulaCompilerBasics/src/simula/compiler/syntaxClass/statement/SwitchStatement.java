...
public final class SwitchStatement extends Statement {
	...
	SwitchStatement(final AstBuilder astBuilder) {
		super(astBuilder);
		astBuilder.consume(KeyWord.SWITCH); //  (add it to 'current tree')

//		if (Option.internal.TRACE_PARSE)	PsiParse.TRACE("Parse SwitchStatement: line="+line);
		PsiParse.expect(astBuilder, KeyWord.BEGPAR);
		lowKey = Expression.expectExpression(astBuilder, "lowkey");
		PsiParse.expect(astBuilder, KeyWord.COLON);
		hiKey = Expression.expectExpression(astBuilder, "hikey");
		PsiParse.expect(astBuilder, KeyWord.ENDPAR);
		switchKey = Expression.expectExpression(astBuilder, "switch");
		switchKey.backLink=this;
		PsiParse.expect(astBuilder, KeyWord.BEGIN);
		has_NONE_case=false;
		while (PsiParse.accept(astBuilder, KeyWord.WHEN)) {
//			astBuilder.startSubtree(PsiTree.Kind.switchStatement, "WhenCase");
//			Util.STOP();

			Vector<SwitchInterval> caseKeyList=new Vector<SwitchInterval>();
			if (PsiParse.accept(astBuilder, KeyWord.NONE)) {
				caseKeyList.add(null);
				if(has_NONE_case) Util.syntaxError(astBuilder, "NONE Case is already used");
				has_NONE_case=true;
			}
			else {
				caseKeyList.add(expectCasePair(astBuilder));
				while(PsiParse.accept(astBuilder, KeyWord.COMMA)) caseKeyList.add(expectCasePair(astBuilder));
			}
			PsiParse.expect(astBuilder, KeyWord.DO);
			
			astBuilder.startSubtree(PsiTree.Kind.switchStatement, "WhenCase");
			Statement statement = Statement.acceptStatement(astBuilder);
			astBuilder.doneSubtree(PsiTree.Kind.switchStatement, statement);
			
			PsiParse.accept(astBuilder, KeyWord.SEMICOLON);
			SwitchWhenPart whenPart = new SwitchWhenPart(caseKeyList, statement);
			switchCases.add(whenPart);
			
//			astBuilder.doneSubtree(PsiTree.Kind.switchStatement, whenPart);
		}
		PsiParse.expect(astBuilder, KeyWord.END);
	...
	private SwitchInterval expectCasePair(final AstBuilder astBuilder) {
		Expression lowCase=Expression.expectExpression(astBuilder, "case");
		Expression hiCase=null;
		if(PsiParse.accept(astBuilder, KeyWord.COLON)) hiCase=Expression.expectExpression(astBuilder, "hicase");
