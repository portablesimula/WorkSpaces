...
public final class ConnectionStatement extends Statement {
	...
	ConnectionStatement(final AstBuilder astBuilder) {
		super(astBuilder);
		astBuilder.consume(KeyWord.INSPECT); //  (add it to 'current tree')

		if (Option.internal.TRACE_PARSE)
			PsiParse.TRACE("Parse ConnectionStatement");
		objectExpression = Expression.expectExpression(astBuilder, "connected object");
		objectExpression.backLink = this;
		String ident = "_inspect_" + firstLineNumber() + '_' + (SEQUX++);
		inspectedVariable = new VariableExpression(astBuilder, ident);
		DeclarationScope scope = Global.getCurrentScope();
		inspectVariableDeclaration = new InspectVariableDeclaration(astBuilder, Type.Ref("RTObject"), ident, scope, this);
...
//		IO.println("NEW ConnectionStatement: add inspectVariableDeclaration to "+scope);
		...
		if (PsiParse.accept(astBuilder, KeyWord.DO)) {
			hasDoPart = true;
			ConnectionBlock connectionBlock = new ConnectionBlock(astBuilder, inspectedVariable, null);
			DeclarationScope prevScope = Global.getCurrentScope();
			Global.setScope(connectionBlock);
			Statement statement = Statement.acceptStatement(astBuilder);
			Global.setScope(prevScope);
			
			connectionPart.add(new ConnectionDoPart(astBuilder, this,connectionBlock, statement));
			connectionBlock.end();
		} else {
			astBuilder.startSubtree(PsiTree.Kind.statement, "Connection WHEN");
			while (PsiParse.accept(astBuilder, KeyWord.WHEN)) {
//				astBuilder.startSubtree(PsiTree.Kind.statement, "Connection WHEN");
				String classIdentifier = PsiParse.expectIdentifier(astBuilder).edText();
				PsiParse.expect(astBuilder, KeyWord.DO);
				ConnectionBlock connectionBlock = new ConnectionBlock(astBuilder, inspectedVariable, classIdentifier);
				hasWhenPart = true;
				Statement statement = Statement.acceptStatement(astBuilder);
				ConnectionWhenPart whenPart = new ConnectionWhenPart(astBuilder, this,classIdentifier, connectionBlock, statement);
				astBuilder.doneSubtree(PsiTree.Kind.statement, whenPart);
				astBuilder.startSubtree(PsiTree.Kind.statement, "Connection WHEN");
				connectionPart.add(whenPart);
				connectionBlock.end();
			}
			astBuilder.dropSubtree(PsiTree.Kind.statement, "Connection WHEN");

		}
		if(!(hasDoPart | hasWhenPart)) Util.syntaxError(astBuilder, "Incomplete Inspect statement: "+objectExpression + ", missing DO or WHEN");
		Statement otherwise = null;
		if (PsiParse.accept(astBuilder, KeyWord.OTHERWISE)) otherwise = Statement.acceptStatement(astBuilder);
