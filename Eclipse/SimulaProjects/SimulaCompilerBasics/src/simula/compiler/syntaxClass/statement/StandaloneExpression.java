...
//	StandaloneExpression(final PsiBuilder astBuilder, final int line,final Expression expression) {
//		super(line);
	StandaloneExpression(final AstBuilder astBuilder, final Expression expression) {
		super(astBuilder);
//		IO.println("\nNEW StandaloneExpression: expr="+expression);
//		astBuilder.printPSI("NEW StandaloneExpression: expr="+expression);

		this.expression = expression;
		if (Option.internal.TRACE_PARSE) {
			Util.TRACE("Line "+firstLineNumber()+": StandaloneExpression: "+this);
			IO.println("Line "+firstLineNumber()+": StandaloneExpression: "+this+"   "+astBuilder.getCurrentLexerToken());
		}
		LexToken prevToken = null;
		while ((prevToken = PsiParse.acceptParserToken(astBuilder, KeyWord.ASSIGNVALUE, KeyWord.ASSIGNREF)) != null) { 
//			IO.println("NEW StandaloneExpression: prevToken="+prevToken);
			astBuilder.startSubtree(PsiTree.Kind.expression, "AssignmentOperation");
			this.expression = new AssignmentOperation(astBuilder, this.expression, prevToken.keyWord, expectStandaloneExpression(astBuilder));
			astBuilder.doneSubtree(PsiTree.Kind.expression, this);
		}		
		
//		IO.println("\nEND NEW StandaloneExpression: expr="+expression);
//		astBuilder.printPSI("END NEW StandaloneExpression: expr="+expression);

	}

//	@Override
//	public PsiTree getPsiTree() {
//		PsiTree psiTree = (astBuilder == null)? null : astBuilder.psiTree;
...
//	}
	...
	private static Expression expectStandaloneExpression(AstBuilder astBuilder) { 
		Expression retExpr=Expression.expectExpression(astBuilder, "standalone");
		LexToken prevToken = null;
		while ((prevToken = PsiParse.acceptParserToken(astBuilder, KeyWord.ASSIGNVALUE,KeyWord.ASSIGNREF)) != null) {
			int opr=prevToken.keyWord;
			retExpr=new AssignmentOperation(astBuilder, retExpr, opr, expectStandaloneExpression(astBuilder));
		}
