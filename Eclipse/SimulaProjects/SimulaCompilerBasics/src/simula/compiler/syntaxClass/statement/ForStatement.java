...
public final class ForStatement extends Statement {
	...
	ForStatement(final AstBuilder astBuilder) {
		super(astBuilder);
		astBuilder.consume(KeyWord.FOR); //  (add it to 'current tree')

		if (Option.internal.TRACE_PARSE)
			PsiParse.TRACE("Parse ForStatement");
		controlVariable = new VariableExpression(astBuilder, PsiParse.expectIdentifier(astBuilder).edText());
		LexToken prevToken = PsiParse.getCurrentParserToken(astBuilder);
		if (!PsiParse.accept(astBuilder, KeyWord.ASSIGNVALUE))
			PsiParse.expect(astBuilder, KeyWord.ASSIGNREF);
		assignmentOperator = prevToken.keyWord;
		do {
			forList.add(expectForListElement(astBuilder));
		} while (PsiParse.accept(astBuilder, KeyWord.COMMA));
		PsiParse.expect(astBuilder, KeyWord.DO);
		Statement doStatement = Statement.acceptStatement(astBuilder);
		if (doStatement == null) {
			Util.syntaxError(astBuilder, "No statement following DO in For statement");
			doStatement = DummyStatement.ofImplicit(astBuilder);
		}
	...
	private ForListElement expectForListElement(AstBuilder astBuilder) {
		if (Option.internal.TRACE_PARSE)
			PsiParse.TRACE("Parse ForListElement");
		Expression expr1 = Expression.expectExpression(astBuilder, "for list");
		if (PsiParse.accept(astBuilder, KeyWord.WHILE))
			return (new ForWhileElement(astBuilder, this, expr1, Expression.expectExpression(astBuilder, "for while")));
		if (PsiParse.accept(astBuilder, KeyWord.STEP)) {
			Expression expr2 = Expression.expectExpression(astBuilder, "for step");
			PsiParse.expect(astBuilder, KeyWord.UNTIL);
			return (new StepUntilElement(astBuilder, this, expr1, expr2, Expression.expectExpression(astBuilder, "for until")));
		} else
			return (new ForListElement(astBuilder, this, expr1));
	}
