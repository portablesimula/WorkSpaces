...
public final class ActivationStatement extends Statement {
	...
	ActivationStatement(final AstBuilder astBuilder) {
		super(astBuilder);
		LexToken activator = PsiParse.getCurrentParserToken(astBuilder);
		REAC = activator.keyWord == KeyWord.REACTIVATE;
		astBuilder.consume(KeyWord.ACTIVATE, KeyWord.REACTIVATE); //  (add it to 'current tree')
		if (Option.internal.TRACE_PARSE) PsiParse.TRACE("Parse ActivationStatement");
		object1 = Expression.expectExpression(astBuilder, "process");
		object1.backLink = this;
		code = ActivationCode.direct;
		LexToken prevToken = PsiParse.getCurrentParserToken(astBuilder);
		if (PsiParse.accept(astBuilder, KeyWord.AT) || PsiParse.accept(astBuilder, KeyWord.DELAY)) {
			code = (prevToken.keyWord == KeyWord.AT) ? ActivationCode.at : ActivationCode.delay;
			time = Expression.expectExpression(astBuilder, "at/delay");
			time.backLink = this;
			if (PsiParse.accept(astBuilder, KeyWord.PRIOR)) prior = true;
		} else if (PsiParse.accept(astBuilder, KeyWord.BEFORE) || PsiParse.accept(astBuilder, KeyWord.AFTER)) {
			code = (prevToken.keyWord == KeyWord.BEFORE) ? ActivationCode.before : ActivationCode.after;
			object2 = Expression.expectExpression(astBuilder, "before/after");
