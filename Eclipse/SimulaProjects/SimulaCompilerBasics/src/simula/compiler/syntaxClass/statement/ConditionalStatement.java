...
public final class ConditionalStatement extends Statement {
	...
	ConditionalStatement(AstBuilder astBuilder) {
		super(astBuilder);
		int lno = astBuilder.getSourceLineNumber();
//		IO.println("NEW ConditionalStatement: "+astBuilder.getSourceLineNumber());
		if (Option.internal.TRACE_PARSE) Util.TRACE("Line " + lno + ": BEGIN IfStatement: ");
		astBuilder.consume(KeyWord.IF); //  (add it to 'current tree')

		condition = Expression.expectExpression(astBuilder, "if-condition");
		PsiParse.expect(astBuilder, KeyWord.THEN);
		Statement elseStatement = null;
		if (PsiParse.accept(astBuilder, KeyWord.ELSE)) {
			thenStatement = DummyStatement.ofImplicit(astBuilder);
			elseStatement = Statement.acceptStatement(astBuilder);
		} else {
		    thenStatement = Statement.acceptStatement(astBuilder);
		    if (PsiParse.accept(astBuilder, KeyWord.ELSE)) {
			    elseStatement = Statement.acceptStatement(astBuilder);
		    }
		}
		...
		if (Option.internal.TRACE_PARSE)
			Util.TRACE("Line " + astBuilder.getSourceLineNumber() + ": DONE IfStatement started at line: " + lno + ": " + this);
	}
