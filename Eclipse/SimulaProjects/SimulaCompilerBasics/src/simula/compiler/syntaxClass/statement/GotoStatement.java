...
public final class GotoStatement extends Statement {
	...
	GotoStatement(final AstBuilder astBuilder, final int keyWord) {
		super(astBuilder);
		astBuilder.consume(KeyWord.GOTO, KeyWord.GO); //  (add it to 'current tree')
		if(keyWord != KeyWord.GOTO) {
	        if (!PsiParse.accept(astBuilder, KeyWord.TO))
	        	Util.syntaxError(astBuilder, "Missing 'TO' after 'GO'");
		}
		label = Expression.expectExpression(astBuilder, "designational");
