...
@Deprecated
default  <T> T getUserDataUnprotected( Key<T> key) {
...
//@ApiStatus.ScheduledForRemoval
//@Deprecated
default <T> void putUserDataUnprotected( Key<T> key,  T value) {
...
interface Marker extends SyntaxTreeBuilder.Marker {
 ...
 @Override
 default void doneBefore( IElementType type, SyntaxTreeBuilder. Marker before) {
   doneBefore(type, (Marker)before);
 }

 @Override
 default void doneBefore( IElementType type,
                         SyntaxTreeBuilder. Marker before,
                           String errorMessage) {
   doneBefore(type, (Marker) before, errorMessage);
 }

 @Override
 default void errorBefore(  String message, SyntaxTreeBuilder. Marker before) {
   errorBefore(message, (Marker) before);
 }

 void doneBefore( IElementType type,  Marker before);

 void doneBefore( IElementType type,
                          Marker before,
                           String errorMessage);
