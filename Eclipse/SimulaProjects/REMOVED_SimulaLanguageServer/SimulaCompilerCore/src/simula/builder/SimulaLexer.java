...
//    }
    ...
    /// Debug utility
    public void snapShot(String title) {
    	int beg = Math.max(0, currentPosition - 50); beg = beg - beg%10;
...
//    	IO.println("sourceText:        " + (""+sourceText).replace("\r", "\\r").replace("\n", "\\n"));
//    	IO.println("sourceText:        " + (""+sourceText).replace("\r", "¤").replace("\n", "¤"));
    	IO.println("sourceText:        " + (""+text).replace("\r", "¤").replace("\n", "¤"));
    	IO.println("textEndOffset:     " + textEndOffset + '(' + (textEndOffset-beg) + ')');
    	IO.println("currentLexerToken: " + currentLexerToken);
    	IO.println("currentPosition:   " + currentPosition + '(' + (currentPosition-beg) + ")  With value: " + edCurrent());
//    	IO.println("tokenStartOffset:  " + tokenStartOffset);
//    	IO.println("tokenEndOffset:    " + tokenEndOffset);
    	IO.println("currentColumn:     " + currentColumn);
//    	IO.println("currentLength:     " + currentLength);
...
//        	IO.println("Line " + i + ": start: " + getLineStartPos(i) + ", end: " + end);
        	...
        	IO.println(line + '|' + (""+text).replace("\r", "¤").replace("\n", "¤") + '|');
        }
    }
...
//			IO.println("SimulaLexer.nextToken: currentColumn = "+token.column+" FROM POP TOKEN");
...
//		IO.println("SimulaLexer.nextToken: currentColumn="+currentColumn+", currentPosition=" + currentPosition + ", tokenStartPos="+tokenStartPos);
		...
		simBuilder.tokenList.add(token);
...
//				case EOF_MARK: return(null);
    		    case '=':
		            ...
		            if(getNext() == '=')   return(newKeyWordToken(KeyWord.NER));
		            	...
		            	pushBackPos(1);
		            	LexToken token = newKeyWordToken(KeyWord.EQ);
		        		Util.syntaxError(simBuilder, token, error);
		            	return token;
		            }
	            case '>':
...
//		IO.println("SimulaLexer.scanWhiteSpace: END Current: " + edCurrent());
    ...
    private LexToken scanNumber() {
    	...
    	try {
    		...
    		LexToken token = newIntegerToken(res);
    		Util.syntaxError(simBuilder, token, "Integer number out of range: "+result);
    		return token;
    	}
    ...
    private LexToken scanDotDigit(StringBuilder number) {
    	...
    	try {
    		...
    		Util.syntaxError(simBuilder, token, "Illegal number: "+result);
    		return token;
    	}
    }
...
//    	return(newKeyWordToken(KeyWord.CHARACTERKONST,Character.valueOf(result)));
    	...
    	LexToken token = newCharacterToken(result);
    ...
    private LexToken scanTextConstant() {
        ...
        int nPhrase = 0; // Number of simple string phrases
    	LOOP:while(true) {
    		// Scan simple-string:
...
//    			IO.println("SimulaLexer.scanTextConstant: CHECK character: " + edChar((char) current));
    			if(current=='!') {
	        	    ...
	        	    tokenQueueAdd("scanTextConstant - CRLF", scanNewLine());
	        	    currentColumn = 0;
    				...
    				LexToken token = newSimpleStringToken(result);
    				Util.syntaxError(simBuilder, token, "Text constant is not terminated.");
    	    	    tokenQueueAdd("scanTextConstant - TOKEN-EOF", token);
    				break LOOP;
    	    ...
    	    if(nPhrase > 0)
    	    	Util.warning(simBuilder, token, "Illegal Text constant. Simple string span mutiple source lines. See Simula Standard 1.6");
    	    tokenQueueAdd("scanTextConstant - TOKEN-2", token);
        	sb=new StringBuilder();
    		getNext(); // consume "
...
//    	    Util.STOP();
	...
	private LexToken OLD_scanComment() {
		String error = null;
		...
		if (current == ';')
			...
			error = "Comment is not terminated with ';'.";
			pushBackPos(1);
		}
		...
		LexToken token = newKeyWordToken(KeyWord.COMMENT_KEY);
		if(error != null) Util.syntaxError(simBuilder, token, error);
		return token;
	}
	   private static boolean TESTING_SCAN_COMMENT = true;
//	   private static int SEQU = 0;
	    private LexToken scanComment() {
//	    	this.snapShot("BEGIN scanComment");
//	    	Util.STOP();
	    	
	        LexToken endToken = newKeyWordToken(KeyWord.COMMENT_KEY);
	        if(TESTING_SCAN_COMMENT) IO.println("LexToken.scanComment: endToken="+endToken);
			IO.println("SimulaLexer.scanComment: currentColumn = "+currentColumn+" + "+endToken.length + " = "+(currentColumn + endToken.length));
...
//	        LOOP:while (current != EOF_MARK) {
	        LOOP:while (true) {
	        	if(current == EOF_MARK) {
	                IO.println("\n\n\n\nLexToken.scanComment: BEGIN TREAT EOF_MARK: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
	                IO.println("LexToken.scanComment: AT EOF_MARK: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
	                ...
	                if(lng > 0) {
	                    ...
	                    tokenQueueAdd("scanComment-EOF_MARK", token);
	                }
	        		break LOOP;
	        	}
	        	
	        	getNext();
	        	if(TESTING_SCAN_COMMENT) IO.println("LexToken.scanComment: current="+current+":'"+(""+(char)current).replace("\r", "\\r").replace("\n", "\\n")+"'");
	        	
	    		if(current == '\r' && nextCharIs('\n')) {
	            	if(TESTING_SCAN_COMMENT) IO.println("LexToken.scanComment: GOT CRLF");
	                IO.println("\n\n\n\nLexToken.scanComment: BEGIN TREAT NEWLINE: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
...
//	                IO.println("LexToken.scanComment: AT NEWLINE: lng="+lng);
	                if(lng > 0) {
...
//	                    IO.println("LexToken.scanComment: AT NEWLINE:TEXT: currentColumn: " + currentColumn);
//	                    IO.println("LexToken.scanComment: AT NEWLINE:TEXT: currentPosition: " + currentPosition);
//	                    IO.println("LexToken.scanComment: AT NEWLINE:TEXT: tokenStartPos: " + tokenStartPos);
//	                    IO.println("LexToken.scanComment: AT NEWLINE:TEXT: currentPosition - tokenStartPos: " + (currentPosition - tokenStartPos));
//	                    IO.println("LexToken.scanComment: AT NEWLINE:TEXT: commentText: \"" + sourceText.subSequence(tokenStartPos, tokenStartPos + lng)+'"');
//	                    IO.println("LexToken.scanComment: AT NEWLINE:TEXT: lng=" + lng +", currentPosition - tokenStartPos: " + (currentPosition - tokenStartPos));
	                    if(Option.LEX_VERIFY) {
...
//		                    if(sourceText.charAt(currentPosition) != '\n') Util.IERR("IMPOSSIBLE: " + edCurrent());
		                    if(sourceText.charAt(currentPosition) != '\r') Util.IERR("IMPOSSIBLE: " + edCurrent());
	                    }
...
//	                    this.snapShot("");
	                    ...
	                    getNext(); getNext(); // Leser første tegn etter comment text, altså et CRLF tegn
	                    if(Option.LEX_VERIFY) {
		                    if(current != '\n') Util.IERR("IMPOSSIBLE");
	                ...
	                if(current != '\n') Util.IERR("IMPOSSIBLE");
	        	    tokenQueueAdd("scanComment - CRLF", scanNewLine());
	        	    currentColumn = 0;
...
//    				Util.STOP();
	            	...
	            	if(TESTING_SCAN_COMMENT) IO.println("LexToken.scanComment: GOT NEWLINE");
	                IO.println("\n\n\n\nLexToken.scanComment: BEGIN TREAT NEWLINE: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
...
//	                    this.snapShot("");
	                ...
	                if(current != '\n') Util.IERR("IMPOSSIBLE");
	        	    tokenQueueAdd("scanComment - NEWLINE", scanNewLine());
	        	    currentColumn = 0;
	            } else if (current == ';') {
	                IO.println("\n\n\n\nLexToken.scanComment: BEGIN TREAT SEMICOLON: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
	                IO.println("LexToken.scanComment: AT SEMICOLON: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
	                int lng = currentPosition - tokenStartPos - 1;
...
//	                    this.snapShot("scanComment-SEMICOLON");
//	                	Util.STOP();
	                }
	                tokenQueueAdd("scanComment-SEMICOLON", newKeyWordToken(tokenStartPos, KeyWord.SEMICOLON));
	                break LOOP;
	            } else {
	            	if(TESTING_SCAN_COMMENT) IO.println("LexToken.scanComment: GOT OTHER="+current+":'"+(""+(char)current).replace("\r", "\\r").replace("\n", "\\n")+"'");
//	                lastLine = nextLineNumber;
	            }
	        }

	        if(TESTING_SCAN_COMMENT) {
		        IO.println("SimulaLexer.scanComment: endToken: " + endToken);
		        IO.println("SimulaLexer.scanComment: TOKEN QUEUE AFTER END -----------------------------------------------------------------------");
		        IO.println("SimulaLexer.scanComment: END TOKEN: " + endToken);
		        printQueue();
		        IO.println("SimulaLexer.scanComment: TOKEN QUEUE AFTER END -----------------------------------------------------------------------");
	        }

	    	this.snapShot("END scanComment");
//	    	Util.STOP();
	...
	/// </pre>
	/// @return a Comment Token
	    private static boolean TESTING_SCAN_END_LINE = true;
...
//		Util.STOP();
        LOOP:while (true) {
        	if(current == EOF_MARK) {
                IO.println("\n\n\n\nLexToken.scanCommentToEndOfLine: BEGIN TREAT EOF_MARK: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
                IO.println("LexToken.scanCommentToEndOfLine: AT EOF_MARK: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
                int lng = currentPosition - tokenStartPos - 1;
                IO.println("LexToken.scanCommentToEndOfLine: AT EOF_MARK: lng="+lng);
                if(lng > 0) {
                    if(currentPosition != textEndOffset) Util.IERR("IMPOSSIBLE");
                    LexToken token = newKeyWordToken(tokenStartPos, KeyWord.COMMENT_TEXT);
                    tokenQueueAdd("scanCommentToEndOfLine-EOF_TEXT", token);
                }
//                tokenQueueAdd("scanCommentToEndOfLine-EOF_TEXT", newKeyWordToken(tokenStartPos, KeyWord.NEWLINE));
                tokenQueueAdd("scanCommentToEndOfLine-EOF_TEXT", newKeyWordToken(tokenStartPos, KeyWord.EOF));
        	    currentColumn = 0;
        		break LOOP;
        	}

        	getNext();
        	if(TESTING_SCAN_END_LINE) IO.println("LexToken.scanCommentToEndOfLine: current="+current+":'"+(""+(char)current).replace("\r", "\\r").replace("\n", "\\n")+"'");
        	if(current == '\r' && nextCharIs('\n')) {
            	if(TESTING_SCAN_END_LINE) IO.println("LexToken.scanCommentToEndOfLine: GOT CRLF");
                IO.println("\n\n\n\nLexToken.scanCommentToEndOfLine: BEGIN TREAT NEWLINE: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
                
                int lng = currentPosition - tokenStartPos - 1;
//                IO.println("LexToken.scanCommentToEndOfLine: AT NEWLINE: lng="+lng);
                if(lng > 0) {
                	pushBackPos(1);
                	if(Option.LEX_VERIFY) {
                		if(lng != (currentPosition - tokenStartPos)) Util.IERR("IMPOSSIBLE: lng=" + lng +", currentPosition - tokenStartPos: " + (currentPosition - tokenStartPos));
                		if(sourceText.charAt(currentPosition) != '\r') Util.IERR("IMPOSSIBLE");
                	}
                	tokenQueueAdd("scanCommentToEndOfLine-NEWLINE", newKeyWordToken(tokenStartPos, KeyWord.COMMENT_TEXT));
                ...
                if(current != '\n') Util.IERR("IMPOSSIBLE");
                tokenQueueAdd("scanCommentToEndOfLine - NEWLINE", scanNewLine());
                currentColumn = 0;
                break LOOP;
        	} else if (current == '\n') {
        		if(TESTING_SCAN_END_LINE) IO.println("LexToken.scanCommentToEndOfLine: GOT NEWLINE");
        		IO.println("\n\n\n\nLexToken.scanCommentToEndOfLine: BEGIN TREAT NEWLINE: currentPosition="+currentPosition+", tokenStartPos="+tokenStartPos);
        		int lng = currentPosition - tokenStartPos - 1;
        		IO.println("LexToken.scanCommentToEndOfLine: AT NEWLINE: lng="+lng);
//        		Util.STOP();
        ...
        if(TESTING_SCAN_END_LINE) {
        	IO.println("SimulaLexer.scanCommentToEndOfLine: TOKEN QUEUE AFTER END -----------------------------------------------------------------------");
        	printQueue();
        	IO.println("SimulaLexer.scanCommentToEndOfLine: TOKEN QUEUE AFTER END -----------------------------------------------------------------------");
        }
        LexToken token = tokenQueue.pop();
        IO.println("SimulaLexer.scanCommentToEndOfLine: " + token);
//        Util.STOP();
        return token;
	}
...
//		IO.println("SimulaLexer.scanEndComment: currentColumn = "+currentColumn+" + "+endToken.length + " = "+(currentColumn + endToken.length));
		currentColumn = currentColumn + endToken.length;
    	tokenStartPos = currentPosition;
...
//		IO.println("SimulaLexer.scanEndComment: current = "+current+" "+edCurrent());
//        LOOP:while (current != EOF_MARK) {
        LOOP:while (true) {
        	if(current == EOF_MARK) {
...
//                IO.println("LexToken.scanEndComment: AT EOF_MARK: lng="+lng);
                if(lng > 0) {
                    ...
                    tokenQueueAdd("scanEndComment-EOF_TEXT", token);
                }
//                tokenQueueAdd("scanEndComment-EOF_TEXT", newKeyWordToken(tokenStartPos, KeyWord.NEWLINE));
                tokenQueueAdd("scanEndComment-EOF_TEXT", newKeyWordToken(tokenStartPos, KeyWord.EOF));
        	    currentColumn = 0;
        		break LOOP;
        	}
    		...
    		if(current == '\r' && nextCharIs('\n')) {
            	if(TESTING_SCAN_END) IO.println("LexToken.scanEndComment: GOT CRLF");
...
//                IO.println("LexToken.scanEndComment: AT SEMICOLON: lng="+lng);
                if(lng > 0) {
                    ...
                    LexToken token = newKeyWordToken(tokenStartPos, KeyWord.COMMENT_TEXT);
                    if(nPhrase > 1) Util.warning(simBuilder, token, "END comment spans multiple lines");
                    tokenQueueAdd("scanEndComment-SEMICOLON", token);
...
//		IO.println("SimulaLexer.tokenQueueAdd: "+debugName+" currentColumn = "+currentColumn+" + "+token.length + " = "+(currentColumn + token.length));
		currentColumn = currentColumn + token.length;
    	tokenStartPos = currentPosition;
	    currentLexerToken = token;
	}
