...
//    private int lastAdvancedLineNumber; // Used by PSI VERIFIER
...
//		IO.println("SimulaLexer.resetCheckPoint: nextLineNumber: " + nextLineNumber + ", tokenStartLine: " + tokenStartLine);
...
//    	IO.println("sourceText:        " + (""+sourceText).replace("\r", "\\r").replace("\n", "\\n"));
    ...
    public SimulaLexer(PsiBuilder psiBuilder) {
...
//    		IO.println("SimulaLexer.advance(from queue): nextLineNumber: " + nextLineNumber + ", tokenStartLine: " + tokenStartLine);
    		...
    		if (currentPosition >= textEndOffset) {
    			...
    			if(EOF != null) {
    				...
    				Util.syntaxError(psiBuilder, prevParserToken, "Incomplete program: Attempt to advance Lexer beyond end of source text");
    			}
    			EOF = prevParserToken;
    			// return;
    		} else {
...
//	            case '%': return scanDirectiveLine();
                ...
                case '=':
                    ...
                    backStep(1);
                    return new KeyWordToken(tokenStartLine, sourceText, tokenStartOffset, currentPosition, KeyWord.EQ);
...
//        return new Identifier(tokenStartLine, sourceText, tokenStartOffset, currentPosition);
    ...
    private LexToken scanNumber() {
        ...
        try {
            ...
            LexToken token = new IntegerConst(tokenStartLine, sourceText, tokenStartOffset, currentPosition, 0);
            Util.syntaxError(psiBuilder, token, "Integer number out of range: " + token.edText());
            return token;
        }
    ...
    private LexToken scanDotDigit(StringBuilder number) {
        ...
        try {
            ...
            LexToken token = new RealConst(tokenStartLine, sourceText, tokenStartOffset, currentPosition, 0);
            Util.syntaxError(psiBuilder, token, "Illegal number: " + token.edText());
            return token;
        }
    }
    ...
    private LexToken scanCharacterConstant() {
        ...
        LexToken token = new CharacterConst(tokenStartLine, sourceText, tokenStartOffset, currentPosition, result);
        if(err != null) Util.syntaxError(psiBuilder, token, err);
        return token;
    }
...
//	                currentPosition--;
      ...
      LexToken token = new SimpleString(tokenStartLine, sourceText, tokenStartOffset, currentPosition, result);
      if(err != null) Util.syntaxError(psiBuilder, token, err);
      return token;
    }
...
//            pushBack( current);
    ...
    private LexToken scanDirectComment() {
		LexToken commentToken = new KeyWordToken(tokenStartLine, sourceText, tokenStartOffset, currentPosition, KeyWord.COMMENT_KEY);
    	tokenStartOffset = currentPosition;
		while(current != ';') {
			if(current == '\n') {
	        	tokenQueueAdd("scanDirectComment - TEXT", new KeyWordToken(tokenStartLine, sourceText, tokenStartOffset, currentPosition-1, KeyWord.COMMENT_TEXT));
	        	tokenQueueAdd("scanDirectComment - NEWLINE", new KeyWordToken(tokenStartLine++, sourceText, currentPosition-1, currentPosition, KeyWord.NEWLINE));
	        	tokenStartOffset = currentPosition;
			}
			getNext(); // Updates current and currentPosition
		}
//    	tokenQueueAdd("scanDirectComment - TEXT", new KeyWordToken(tokenStartLine, sourceText, tokenStartOffset, currentPosition, KeyWord.COMMENT_TEXT));
...
//			IO.println("SimulaLexer.scanCommentToEndOfLine: nextLineNumber: " + nextLineNumber + ", tokenStartLine: " + tokenStartLine);
        ...
        return new KeyWordToken(tokenStartLine, sourceText, tokenStartOffset, currentPosition, KeyWord.COMMENT_TEXT);
    }
