...
//	    textDocumentService.didOpen(didOpenTextDocumentParams);
	    ...
	    String uri = demoUri;
	    int version = 0;
	    DocumentManager.didOpen(uri, version, program);
...
//	    System.out.println("SimulaDebugClient.semanticTokens: " + semanticTokens);
	    ...
	    if (semanticTokens != null) {
	    	...
	    	System.out.println("Received token data size: " + data.size());
	    	int i = 0;
	    	while(i < data.size()) {
                int deltaLine = data.get(i++);
                int deltaChar = data.get(i++);
                int length = data.get(i++);
                int tokenTypeIndex = data.get(i++);
                int tokenModifiersBitmask = data.get(i++);
