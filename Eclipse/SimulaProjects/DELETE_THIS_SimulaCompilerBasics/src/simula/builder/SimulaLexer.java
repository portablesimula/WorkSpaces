...
public final class SimulaLexer { // extends DefaultScanner { 
...
//	}
	public SimulaLexer(final CharSequence sourceText) {
...
//                int lng = currentPosition - commentStartPos;
...
//                    LexToken token = newKeyWordToken(commentStartPos, KeyWord.COMMENT_TEXT);
...
//                    LexToken commentToken = newKeyWordToken(KeyWord.COMMENT_TEXT);
//            	    tokenQueueAdd("scanEndComment - NEWLINE:TEXT", commentToken);
//        			currentColumn = currentColumn + commentToken.length;
//        			currentPosition++;
//        	    	tokenStartPos = currentPosition;
...
//            	    tokenQueueAdd("scanEndComment - NEWLINE", newLineToken);
//        			currentColumn = currentColumn + newLineToken.length;
//        	    	tokenStartPos = currentPosition;
...
//                    if(lng > 14) Util.STOP();
    			...
    			IO.println("SimulaLexer.scanCommentToEndOfLine: currentLineNumber: " + currentLineNumber);
    			
//            	commentStartPos = currentPosition; // ????????????
...
//    private int mayBe_AddCommentToken_ToTokenQueue(int commentStartPos, int nPhrase) {
//        if(current != EOF_MARK) currentPosition--;
//        if(currentPosition > tokenStartOffset) {
//        	tokenQueueAdd("scanEndComment - SEMICOLON", commentStartPos, KeyWord.COMMENT_TEXT);
//            tokenStartOffset = currentPosition;
//            if(++nPhrase == 2) {
//            	int lno = Global.sourceLineNumber;
//            	Global.sourceLineNumber = currentLineNumber;
...
//	private void tokenQueueAdd(String debugName, int tokenStartPos, int keyWord) {
...
//		LexToken token = new KeyWordToken(tokenStartLine, sourceText, currentColumn, currentPosition - currentLineStartPosition, keyWord, getTokenDebugText());
        LexToken token = newKeyWordToken(tokenStartPos, keyWord);
...
//			if(current < 32) current = ' '; // Whitespace
    	}
