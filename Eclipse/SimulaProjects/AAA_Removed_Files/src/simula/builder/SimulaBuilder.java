...
//		this.sourceDocumentItem = sourceDocumentItem;
	...
	public void start(CharSequence sourceText) {
		if(Option.TESTING_USE_SCANNER) {
			this.sourceText = sourceText;
			tokenList = new ArrayList<>();
			lexTokenRange = astRoot = new LexTokenRange(null);
	        scanner = new SimulaScanner(sourceText);
//	        scanner.start(sourceText);			
		} else {
			this.sourceText = sourceText;
			tokenList = new ArrayList<>();
			lexTokenRange = astRoot = new LexTokenRange(null);
...
//		IO.println("PsiBuilder.doneTokenRange: syntaxElement="+syntaxElement);
	...
	public void advanceLexer() {
		...
		if(Option.TESTING_NEW_LEXER) {
			lexer.advance();                                // And the advance the lexer.				
		} else {
//			lexer.OLD_advance();                                // And the advance the lexer.	
		}
	}
