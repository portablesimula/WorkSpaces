...
//	private static final Logger LOG = Logger.getInstance(PsiBuilderImpl.class);
	...
	private PsiBuilderImpl(@Nullable Project project,
		...
		LOG.println("NEW PsiBuilderImpl: ");
		LOG.println("NEW PsiBuilderImpl: myProduction: "+myProduction.getClass().getSimpleName()+" "+myProduction);
		myProject = project;
		...
		LOG.println("NEW PsiBuilderImpl: JUST BEFORE: performLexing");
		TokenSequence tokens = performLexing(parentCachingNode);
		LOG.println("NEW PsiBuilderImpl: performLexing: tokens="+tokens);
		myLexStarts = tokens.lexStarts;
		myLexTypes = tokens.lexTypes;
		myLexemeCount = tokens.lexemeCount;
		LOG.println("NEW PsiBuilderImpl: AFTER: performLexing: myLexStarts="+myLexStarts+", myLexTypes="+myLexTypes+", myLexemeCount="+myLexemeCount);
		if (DIAGNOSTICS != null) {
	...
	private @NotNull TokenSequence performLexing(@Nullable Object parentCachingNode) {
		LOG.println("PsiBuilderImpl.performLexing: parentCachingNode="+parentCachingNode);
		...
		if (parentCachingNode instanceof LazyParseableToken && shouldReuseCollapsedTokens(((LazyParseableToken)parentCachingNode).getTokenType())) {
			LOG.println("PsiBuilderImpl.performLexing: parentCachingNode instanceof LazyParseableToken");
			...
			LOG.println("PsiBuilderImpl.performLexing: parentCachingNode instanceof LazyParseableElement");
		...
		LOG.println("PsiBuilderImpl.performLexing: fromParent="+fromParent);
		if (fromParent != null) {
...
//		}
		...
		int getLexemeIndex(boolean done) {
			return done ? getEndIndex() : getStartIndex();
		}
...
//			System.out.println("PsiBuilderImpl.done: "+type.getClass()+" "+type);
	...
	private static final class LazyParseableToken extends TokenRange implements LighterLazyParseableNode {
		...
		public @NotNull FlyweightCapableTreeStructure<LighterASTNode> parseContents() {
			if(true) throw new RuntimeException("PsiBuilderImpl.parseContents: SJEKK DETTE");
...
//		Util.IERR("PREPARE TREE - STOP HER");
		...
		if (myCurrentLexeme < myLexemeCount) {
			...
			LOG.aerror("Tokens " + missed + " were not inserted into the tree. "
					+ (myPsiFile == null
					? ""
		...
		if (rootMarker.getEndIndex() < myLexemeCount) {
			...
			LOG.aerror("Tokens " + missed + " are outside of root element \"" + rootMarker.myType + "\".",
...
//	    System.out.println("Main.printAST: child="+child);
	...
	private void balanceWhiteSpaces() {
		...
		for (int i = 1, size = myProduction.size() - 1; i < size; i++) {
			...
			boolean recursive = binder.isRecursive();
	...
	public static final class MyTreeStructure implements FlyweightCapableTreeStructure<LighterASTNode> {
		...
		public int getChildren(@NotNull LighterASTNode item, @NotNull Ref<LighterASTNode[]> into) {
			if (item instanceof LazyParseableToken) {
				if(true) throw new RuntimeException("PsiBuilderImpl.getChildren: SJEKK DETTE");
