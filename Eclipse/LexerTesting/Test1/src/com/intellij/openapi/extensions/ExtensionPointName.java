...
// Oversatt fra Kotlin:
//public class ExtensionPointName {
	...
	public class ExtensionPointName<T> extends BaseExtensionPointName<T> {
	    ...
	    public T[] getExtensions() {
	        return getRootPoint().getExtensions();
	    }

	    @NotNull
	    public List<T> getExtensionList() {
	        return getRootPoint().getExtensionList();
	    }
	    ...
	    public void forEachExtensionSafe(@NotNull Consumer<? super T> consumer) {
	        ((ExtensionPointImpl<T>) getPointImpl(null)).processWithPluginDescriptor(true, (extension, pluginDescriptor) -> {
	            consumer.accept(extension);
	            return null; // Kotlin unit equivalent
	        });
	    }

	    @Nullable
	    public T findFirstSafe(@NotNull Predicate<? super T> predicate) {
	        return ExtensionProcessingHelper.findFirstSafe(predicate, getRootPoint().asSequence());
	    }

	    @Nullable
	    public <R> R computeSafeIfAny(@NotNull Function<T, ? extends R> processor) {
	        return ExtensionProcessingHelper.computeSafeIfAny(processor::apply, getRootPoint().asSequence());
	    }
	    ...
	    public List<T> getExtensionsIfPointIsRegistered(@Nullable AreaInstance areaInstance) {
	        ExtensionsArea area = areaInstance != null ? areaInstance.getExtensionArea() : Extensions.getRootArea();
	        if (area == null) return Collections.emptyList();
	        
	        ExtensionPoint<T> point = area.getExtensionPointIfRegistered(getName());
	        return point != null ? point.getExtensionList() : Collections.emptyList();
	    }
	    ...
	    public boolean hasAnyExtensions() {
	        @SuppressWarnings("deprecation")
	        ExtensionPoint<T> point = Extensions.getRootArea().getExtensionPointIfRegistered(getName());
	    ...
	    public <V extends T> V findExtension(@NotNull Class<V> instanceOf) {
	        return getRootPoint().findExtension(instanceOf, false, ThreeState.UNSURE);
	    }

	    @NotNull
	    public <V extends T> V findExtensionOrFail(@NotNull Class<V> exactClass) {
	        V result = getRootPoint().findExtension(exactClass, true, ThreeState.UNSURE);
	        assert result != null;
	        return result;
	    }
	    ...
	    public <V extends T> V findFirstAssignableExtension(@NotNull Class<V> instanceOf) {
	        return getRootPoint().findExtension(instanceOf, true, ThreeState.NO);
	    }

	    @ApiStatus.Internal
	    @NotNull
	    public Iterable<T> getIterable() {
	        return getRootPoint().asSequence().asIterable();
	    }

	    @ApiStatus.Internal
	    public void processWithPluginDescriptor(@NotNull java.util.function.BiConsumer<T, PluginDescriptor> consumer) {
	        ((ExtensionPointImpl<T>) getRootPoint()).processWithPluginDescriptor(false, (extension, descriptor) -> {
	            consumer.accept(extension, descriptor);
	            return null;
	        });
	    }

	    public void addExtensionPointListener(@NotNull ExtensionPointListener<T> listener, @Nullable Disposable parentDisposable) {
	        getRootPoint().addExtensionPointListener(listener, false, parentDisposable);
	    }

	    @ApiStatus.Internal
	    public void addExtensionPointListener(@NotNull CoroutineScope coroutineScope, @NotNull ExtensionPointListener<T> listener) {
	        getRootPoint().addExtensionPointListener(listener, false, coroutineScope);
	    }

	    public void addExtensionPointListener(@NotNull ExtensionPointListener<T> listener) {
	        getRootPoint().addExtensionPointListener(listener, false, null);
	    }

	    public void addExtensionPointListener(@NotNull AreaInstance areaInstance, @NotNull ExtensionPointListener<T> listener) {
