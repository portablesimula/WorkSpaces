...
//			if(Option.internal.TRACE_PARSE) Util.TRACE("Expression: ParseExpression, result="+expr);
//			return(expr);
			throw new RuntimeException("Expression.acceptExpression: NOT IMPL");
		} else return(acceptSimpleExpression(simBuilder));
	} 
...
//			int opr=Parse.prevToken.getKeyWord();
	...
	private static Expression acceptUNIMULDIV(PsiBuilder simBuilder) {
		...
		if((prevToken = Parse.acceptToken(simBuilder, KeyWord.PLUS,KeyWord.MINUS)) != null) {
			...
			else expr=UnaryOperation.create(opr,acceptMULDIV(simBuilder));
...
//		if(Option.internal.TRACE_PARSE) Parse.TRACE("Expression: acceptExpression");
		...
		else if((prevToken = Parse.acceptToken(simBuilder, KeyWord.INTEGERKONST)) != null) expr = new Constant(Type.Integer,((IntegerConst)prevToken).value);
		else if((prevToken = Parse.acceptToken(simBuilder, KeyWord.REALKONST)) != null) expr = Constant.createRealType(((RealConst)prevToken).value);
		else if((prevToken = Parse.acceptToken(simBuilder, KeyWord.BOOLEANKONST)) != null) expr = new Constant(Type.Boolean,((IntegerConst)prevToken).value);
		else if((prevToken = Parse.acceptToken(simBuilder, KeyWord.CHARACTERKONST)) != null) expr = new Constant(Type.Character,((CharacterConst)prevToken).value);
		else if((prevToken = Parse.acceptToken(simBuilder, KeyWord.TEXTKONST)) != null) expr = new Constant(Type.Text,((SimpleString)prevToken).value);
