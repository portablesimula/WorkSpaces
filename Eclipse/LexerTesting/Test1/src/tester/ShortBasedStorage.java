...
import lang.highlighter.ArrayUtil;
...
public class ShortBasedStorage implements DataStorage {
  ...
  public ShortBasedStorage() {
    myData = new short[64];//[INITIAL_SIZE];
  }
  ...
  public void setData(int segmentIndex, int data) {
    if (segmentIndex >= myData.length) {
      myData = ArrayUtil.realloc(myData, SegmentArray.calcCapacity(myData.length, segmentIndex));
    }
  ...
  protected static short [] insert(short [] array,
                                            short [] insertArray, int startIndex, int insertLength, int mySegmentCount) {
