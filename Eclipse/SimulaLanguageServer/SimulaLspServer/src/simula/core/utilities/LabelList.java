...
public class LabelList {
	...
	public LabelList(DeclarationScope declaredIn) {
		...
		if(TRACING) Util.println("NEW " + this);
	}
	...
	public void add(LabelDeclaration lab) {
		if(TRACING) Util.println(ident()+".add: " + lab.identifierValue() + '['+lab.externalIdent+']');
		if(READY_FOR_CODING) Util.IERR("Can't add a new Label when LabelLisit is marked READY_FOR_CODING");
		declaredLabels.add(lab);
		if(TRACING) Util.println(ident()+".add: DONE: LabelList = "+this);
	}
	...
	public List<SwitchCase> getTableSwitchCases(CodeBuilder codeBuilder) {
		...
		if(TRACING) Util.println(ident()+".getTableSwitchCases: "+this);
		if(TRACING) Util.println("LabelList.getTableSwitchCases: "+this);
	...
	public void build_JUMPTABLE(BlockCodeBuilder codeBuilder) {
		if(!READY_FOR_CODING) MAKE_READY_FOR_CODING(codeBuilder);
		if(TRACING) Util.println(ident()+".build_JUMPTABLE: "+this);
		// *******************************************************************************
		// *** JUMPTABLE Case
		// *******************************************************************************
	...
	public void labelBinding(LabelDeclaration label,CodeBuilder codeBuilder) {
		...
		if(TRACING) Util.println(ident()+".labelBinding: labelContext="+labelContext+", codeBuilder="+codeBuilder);
		if(TRACING) Util.println(ident()+".labelBinding: currentList="+currentList);
		SwitchCase switchCase=currentList.tableSwitchCases.get(label.index-1);
		if(TRACING) Util.println(ident()+".labelBinding: "+label+"   SwitchCase="+switchCase);
	...
	public void printTree(final int indent) {
		if(Option.internal.PRINT_SYNTAX_TREE > 2) {
			Util.println(SyntaxElement.edIndent(indent)+this);
		} else {
			Util.println(SyntaxElement.edIndent(indent) + "LabelList with " + (declaredLabels.size()) + " DeclaredLabels ...");
		}
	}
	...
	public void print(String title) {
		...
		Util.println("*** DeclaredIn: "+declaredIn.identifierValue() + "  READY_FOR_CODING="+READY_FOR_CODING);
		StringBuilder sb = new StringBuilder();
		sb.append("*** DeclaredLabels:");
		if(declaredLabelSize() > 0) {
			for(LabelDeclaration lab:getDeclaredLabels()) {
				sb.append(" " + lab.identifierValue() + '[' + lab.declaredIn.externalIdent + ':' + lab.index + ']' + "atLine:" + lab.firstLineNumber());
			}
			Util.println(""+sb);
		} else Util.println(" NONE");
		sb.append("*** AccumLabels:   ");
		if(accumLabelSize() > 0) {
		...
		Util.println("*** DefaultTarget:  "+defaultTarget);
		if(tableSwitchCases != null) {
			for(SwitchCase swc:tableSwitchCases) {
				Util.println("*** SwitchCase:     "+swc);
			}
		}
		Util.println("************ ENDOF LabelList[" +sequ + "]: "+title+" ************\n");
	}
	...
	public static LabelList readLabelList(final DocumentManager documentManager, final AttributeInputStream inpt) throws IOException {
		...
		if(present) {
			...
			if(TRACING)
				Util.println("LabelList.readLabelList: Read Label List: "+n);
			for(int i=0;i<n;i++) {
