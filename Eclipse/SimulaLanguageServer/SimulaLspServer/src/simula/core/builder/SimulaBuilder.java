...
//			Util.println("compilerMode         " + args.compilerMode);
			...
			Util.println("DocumentManager.sourceFileName     " + documentManager.documentUri);
...
////        	Util.println("SimulaBuilder: documentManager.sourceCode: "+documentManager.sourceCode);
...
//    	Util.println("SimulaBuilder: this.syntaxTree: "+simBuilder.syntaxTree); // Root of Syntax Tree
//    	Util.println("SimulaBuilder: this.diagnostics: "+simBuilder.diagnostics);
//    	Util.println("SimulaBuilder: this.tokenList: "+simBuilder.tokenList);
//		
...
//		programModule.doChecking();
		...
		if(DocumentManager.verbose) Util.println("SimulaBuilder.doChecking: " + documentManager.simBuilder.documentManager.sourceName + ": Semantic Checker completed");
...
//		Util.println("PsiBuilder.consume: lexToken=" + KeyWord.edit(lexToken.keyWord));
		boolean OK = false;
		LOOP:for(int keyWord:keyWords) {
//			Util.println("PsiBuilder.consume: TEST: " + KeyWord.edit(keyWord) + " == " + KeyWord.edit(lexToken.keyWord));
			if(lexToken.keyWord == keyWord) {
//				Util.println("PsiBuilder.consume: GOT IT: " + KeyWord.edit(keyWord) + " == " + KeyWord.edit(lexToken.keyWord));
				OK = true; break LOOP;
			}
		}
		if(! OK) {
...
//		Util.println("PsiBuilder.rollBackTo: "+prev);
//		Util.println("PsiBuilder.rollBackTo: CurrentParserToken: "+getCurrentParserToken());
//		Util.println("PsiBuilder.rollBackTo: =========== tokenList: BAKLENGS");
		int n = lexTokenList.size();
		rollBackIndex = -1;
		LOOP:for(int i=n-1;i>=0;i--) {
//			Util.println("PsiBuilder.rollBackTo: token "+i+": "+tokenList.get(i));	
			if(lexTokenList.get(i) == prev) {
//				Util.println("PsiBuilder.rollBackTo: FOUND: " + i + ": " + prev);	
				rollBackIndex = i + 1;
				break LOOP;
			}
		}
...
//		Util.println("PsiBuilder.rollBackTo: END rollBackIndex="+rollBackIndex);
	}
...
//			Util.println("PsiBuilder.getNextParserToken: currentParserToken="+currentParserToken);
...
//		Util.println("\n\nSimulaBuilder.getTextString: nextToken: "+nextToken);
    	...
    	while(getCurrentParserToken() instanceof SimpleString str) {
    		result += str.value;
//    		Util.println("SimulaBuilder.getTextString: RESULT: "+result);
//        	Util.println("SimulaBuilder.getTextString: NEXT TOKEN: "+nextToken);
//    		Util.IERR("SJEKK DETTE");
        	getNextParserToken();
    	}
//    	Util.println("SimulaBuilder.getTextString: RETURN TEXT: ]"+result+"[\n\n");
    	return result;
    }
...
//		if(syntaxTree.mainModule != null) syntaxTree.print(0);
		...
		Util.println("======================================== ENDOF SYNTAX TREE: " + title + " ============================ ");
	}
	...
	public void printTokenList(String title) {
		Util.println("======================================== BEGIN TOKEN LIST: " + title + " ============================ ");
		for(LexToken lexToken:lexTokenList) {
			Util.println(""+lexToken);
		}
		Util.println("======================================== ENDOF TOKEN LIST: " + title + " ============================ ");		
	}
