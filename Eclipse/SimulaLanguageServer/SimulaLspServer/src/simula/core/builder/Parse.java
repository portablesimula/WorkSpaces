...
//				Util.println("PsiParse.accept: " + KeyWord.edit(keyword) + " accepted, nextToken: " + PsiParse.currentLexToken(simBuilder));
				return currentToken;
			}
		return null;
	}
...
//		Util.println("PsiParse.accept_AND_THEN: current="+getCurrentParserToken(simBuilder));
		if(accept(simBuilder, KeyWord.AND_THEN)) {
//			Util.println("PsiParse.accept_AND_THEN: GOT: AND_THEN");
			return true;
		}
		if(accept(simBuilder, KeyWord.AND)) {
			LexToken prv = simBuilder.getCurrentParserToken();
//			Util.println("PsiParse.accept_AND_THEN: MAYBE AND THEN prv="+prv);
			if(accept(simBuilder, KeyWord.THEN)) {
//				Util.println("PsiParse.accept_AND_THEN: GOT: AND THEN prv="+prv);
				return true;
			}
//			Util.println("PsiParse.accept_AND_THEN: FAILED --> ROLLBACK prv="+prv);
...
//		Util.println("PsiParse.accept_AND_ONLY: current="+getCurrentParserToken(simBuilder));
		LexToken prv = simBuilder.getCurrentParserToken();
		if(accept(simBuilder, KeyWord.AND)) {
//			Util.println("PsiParse.accept_AND_ONLY: MAYBE AND THEN: current="+getCurrentParserToken(simBuilder));
			if(accept(simBuilder, KeyWord.THEN)) {
//				Util.println("PsiParse.accept_AND_ONLY: GOT: AND THEN prv="+prv);
				simBuilder.rollBackTo(prv, " is not a single AND without THEN");
				return false;
			}
//			Util.println("PsiParse.accept_AND_ONLY: prv="+prv);
			return true;
		}
		return false;
	}
...
//		Util.println("PsiParse.accept_OR_ELSE: current="+getCurrentParserToken(simBuilder));
		if(accept(simBuilder, KeyWord.OR_ELSE)) {
//			Util.println("PsiParse.accept_OR_ELSE: GOT: OR_ELSE");
			return true;
		}
		if(accept(simBuilder, KeyWord.OR)) {
			LexToken prv = simBuilder.getCurrentParserToken();
//			Util.println("PsiParse.accept_OR_ELSE: MAYBE OR ELSE prv="+prv);
			if(accept(simBuilder, KeyWord.ELSE)) {
//				Util.println("PsiParse.accept_OR_ELSE: GOT: OR ELSE prv="+prv);
				return true;
			}
//			Util.println("PsiParse.accept_OR_ELSE: FAILED --> ROLLBACK prv="+prv);
...
//		Util.println("PsiParse.accept_OR_ONLY: current="+getCurrentParserToken(simBuilder));
		LexToken prv = simBuilder.getCurrentParserToken();
		if(accept(simBuilder, KeyWord.OR)) {
//			Util.println("PsiParse.accept_OR_ONLY: MAYBE OR ELSE prv="+prv);
			if(accept(simBuilder, KeyWord.ELSE)) {
//				Util.println("PsiParse.accept_OR_ONLY: GOT: OR ELSE prv="+prv);
				simBuilder.rollBackTo(prv, " is not a single OR without ELSE");
				return false;
			}
//			Util.println("PsiParse.accept_OR_ONLY: prv="+prv);
			return true;
		}
		return false;
	}
