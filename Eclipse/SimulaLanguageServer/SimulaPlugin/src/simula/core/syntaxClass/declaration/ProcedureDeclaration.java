...
//		Util.println("ProcedureDeclaration.expectProcedureDeclaration: "+proc.identifierValue());
...
//		Util.println("ProcedureDeclaration.expectProcedureDeclaration: BEFORE PARSE PROCEDURE BLOCK");
...
//			Util.println("ProcedureDeclaration.acceptModePart: BEGIN VALUE/NAME");
...
//			Util.println("ProcedureDeclaration.acceptModePart: END VALUE/NAME - Expect SEMICOLON");
			Parse.expect(simBuilder, KeyWord.SEMICOLON);
			return(true);
		}
		return(false);
...
//			Util.println("ProcedureDeclaration.expectSpecificationPart: END TYPE Identifiers - Expect SEMICOLON");
			Parse.expect(simBuilder, KeyWord.SEMICOLON);
			continue LOOP;
		}
		for (Parameter par : proc.parameterList) {
...
//			Util.println("ProcedureDeclaration.expectProcedureBody: START PARSE PROCEDURE BLOCK");
			if (Option.internal.TRACE_PARSE)
...
//			Util.println("ProcedureDeclaration.expectProcedureBody: START PARSE PROCEDURE BODY STATEMENT");
			LexToken token =simBuilder.getCurrentParserToken();
//			Util.println("ProcedureDeclaration.expectProcedureBody: token: " + token);
			if(token.keyWord != KeyWord.SEMICOLON) {
				Statement stm = Statement.acceptStatement(simBuilder);
				proc.statements.add(stm);
			}
//			Util.println("ProcedureDeclaration.expectProcedureBody: END PARSE PROCEDURE BODY STATEMENT");
		}
	}
	...
	public void doJavaCoding(final SimulaCoder simCoder) {
		...
		if (this.isPreCompiledFromFile != null) {
			if(DocumentManager.verbose) Util.println("Skip  doJavaCoding: "+this.identifier+" -- It is read from "+isPreCompiledFromFile);		
		} else {
	...
	private void doProcedureCoding(final SimulaCoder simCoder) {
		...
		ASSERT_SEMANTICS_CHECKED();
		if (this.isPreCompiledFromFile != null) {
			if(DocumentManager.verbose) Util.println("Skip  doProcedureCoding: " + this.identifierValue() + " -- It is read from " + isPreCompiledFromFile);		
			return;
		}
	...
	public byte[] buildClassFile(final SimulaCoder simCoder) {
		...
		if(DocumentManager.verbose) Util.println("ProcedureDeclaration.buildClassFile: "+CD_ThisClass);
	...
	public void printTree(final int indent) {
		...
		Util.println(edTreeIndent(indent) + typeID + "PROCEDURE " + identifierValue() + '[' + externalIdent + "]" + tail);
