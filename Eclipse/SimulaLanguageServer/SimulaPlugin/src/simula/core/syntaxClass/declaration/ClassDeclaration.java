...
//			Util.println("ClassDeclaration.expectClassDeclaration: identifier: "+identifierValue());
//			Util.println("ClassDeclaration.expectClassDeclaration: prev: "+prev);
			if(identifier == null) {
...
//		LexToken first = (identifier != null)? identifier : simBuilder.getPrevParserToken();
	...
	private static void acceptParameterSpecificationPart(final SimulaBuilder simBuilder, final Vector<Parameter> pList) {
		if (Option.internal.TRACE_PARSE)
	...
	private static void acceptProtectionPart(final SimulaBuilder simBuilder, ClassDeclaration cls) {
		while (true) {
	...
	private static void expectClassBody(final SimulaBuilder simBuilder, ClassDeclaration cls) {
		if (Parse.accept(simBuilder, KeyWord.BEGIN)) {
			...
			if(! cls.hasInner()) {
//				Util.println("ClassDeclaration.expectClassBody: ADD IMPLICIT INNER");
...
//			Util.println("ClassDecleration.doChecking: prefixClass: " + prefixClass );
			if(prefixClass == null) {
...
//		Util.println("ClassDeclaration.findRemoteAttributeMeaning: " + ident);
		Declaration decl = scope.findLocalAttribute(ident);
//		Util.println("ClassDeclaration.findRemoteAttributeMeaning: " + decl);
		if (decl != null) {
...
//		printStaticChain("",0);
	...
	public void doJavaCoding(final SimulaCoder simCoder) {
		...
		if (this.isPreCompiledFromFile != null) {
			if(DocumentManager.verbose) Util.println("Skip  doJavaCoding: " + this.identifierValue() + " -- It is read from " + isPreCompiledFromFile);	
			return;
		}
		CoreGlobal.sourceLineNumber = firstLineNumber();
...
//				Util.println("ClassDeclaration.doJavaCoding: externalIdent="+par.externalIdent);
//				Util.STOP();
...
//					Util.warning("ClassDeclaration.buildStatementsBeforeInner: Illegal LNO: "+lno+" "+stm.getClass().getSimpleName()+" "+stm);
	...
	public void printTree(final int indent) {
		...
		Util.println(edTreeIndent(indent) + "CLASS " + identifierValue() + tail + "  PrefixLevel=" + prefixLevel() + prfx + declIn);
