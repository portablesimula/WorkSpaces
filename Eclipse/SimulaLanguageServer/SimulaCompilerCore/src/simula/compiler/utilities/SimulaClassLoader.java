...
public class SimulaClassLoader extends ClassLoader {
	...
	public void loadClass(String name, byte[] bytes) {
		Class<?> clazz = CoreGlobal.simulaClassLoader.findLoadedClass(name);
		if(clazz != null) {
			...
			clazz = CoreGlobal.simulaClassLoader.defineClass(name, bytes, 0, bytes.length);
			CoreGlobal.simulaClassLoader.resolveClass(clazz);
