...
public final class ArithmeticExpression extends Expression {
	...
	private ArithmeticExpression(final SimulaBuilder simBuilder, final Expression lhs, final int opr, final Expression rhs) {
		...
		if (lhs == null) {
			...
			this.lhs = new VariableExpression(simBuilder, "UNKNOWN_");
		} else
			this.lhs = lhs;
		if (rhs == null) {
			Util.syntaxError(simBuilder, "Missing operand before " + KeyWord.edit(opr));
			this.rhs = new VariableExpression(simBuilder, "UNKNOWN_");
		} else
			this.rhs = rhs;
...
//	@Override
//	public Expression evaluate(final SimulaBuilder simBuilder) {
	...
	public void doChecking() {
		...
		switch (opr) {
			case KeyWord.PLUS, KeyWord.MINUS, KeyWord.MUL -> {
				...
				if (this.type == null)
					Util.syntaxError(simBuilder, "Incompatible types in binary operation: " + toString());
			}
