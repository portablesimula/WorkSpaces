...
//			Util.syntaxError(simBuilder, "Expecting Expression");
	...
	/// </pre>
	/// @return an expression
	private static Expression acceptRelation(SimulaBuilder simBuilder) {   // Metode-form      
		Expression expr = acceptAdditiveOperation(simBuilder);
		if(Parse.acceptRelationalOperator(simBuilder))   { 
...
//			int opr=Parse.prevToken.getKeyWord();
	...
	private static Expression acceptBASICEXPR(SimulaBuilder simBuilder) {
		...
		else { String ident=Parse.acceptIdentifier(simBuilder).getText();
		...
		while (Parse.acceptPostfixOprator(simBuilder)) {
