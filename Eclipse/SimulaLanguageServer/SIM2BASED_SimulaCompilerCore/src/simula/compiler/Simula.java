...
public final class Simula {
	...
	private static void help() {
		Util.println(CoreGlobal.simulaReleaseID+" See: https://github.com/portablesimula");
		Util.println("");
	...
	public static void main(String[] argv) {
		...
		CoreGlobal.initSimulaProperties();
	    ...
	    if(!Option.internal.INLINE_TESTING) CoreGlobal.simulaRtsLib=new File(CoreGlobal.releaseHome, "/rts");
	    
		if (fileNames.isEmpty()) {
			// *** STARTING SIMULA EDITOR ***
			CoreGlobal.sampleSourceDir = new File(CoreGlobal.releaseHome, "/samples");
	        ...
	        for(String fileName:fileNames) {
				...
				try {
					...
					InputStreamReader reader = new InputStreamReader(new FileInputStream(file), CoreGlobal._CHARSET);
	...
	private static void printVersion() {
    	String revision=CoreGlobal.getSimulaProperty("simula.revision","?");
    	String dated=CoreGlobal.getSimulaProperty("simula.setup.dated","?");
        String releaseID=CoreGlobal.simulaReleaseID+'R'+revision;
	...
	private static void setOutputDir(final String dir) {
		CoreGlobal.outputDir = new File(dir);
		Util.TRACE("OUTPUT_DIR: " + CoreGlobal.outputDir);
	}
