...
//		Util.println("DeclarationScope.modifyIdentifier: " + identifier);
...
//		Util.println("DeclarationScope.findMeaning: " + this.getClass());
//		Util.println("DeclarationScope.findMeaning: Looking for "+identifierValue()+" in " + ((declaredIn==null)?"NULL":declaredIn.getClass()));
		Meaning meaning = findVisibleAttributeMeaning(identifier);
		if (meaning == null && declaredIn != null) {
//			Util.println("DeclarationScope.findMeaning: Looking for " + identifierValue() + " in "+declaredIn);
			meaning = declaredIn.findMeaning(identifier);
		}
		
		if (meaning == null) {
//			if (!Global.duringParsing) {
//				Util.println("DeclarationScope.findMeaning: Undefined variable: " + identifierValue());
...
//			}
	...
	public void printStaticChain(String title,int details) {
		...
		int lim = 5;//7;
		for(int i=1;i<lim;i++) {
			Util.println("DeclarationScope.printStaticChain: " + scope.edScope());
			if(details > 0) {
				for(Declaration decl:scope.declarationList) {
					Util.println("DeclarationScope.printStaticChain:                  "+decl);			
				}
			}
			scope=scope.declaredIn;
		}
    ...
    public void createJavaClassFile(final SimulaCoder simCoder) throws IOException {
    	if (this.isPreCompiledFromFile != null) {
			if(DocumentManager.verbose) Util.println("Skip  buildClassFile: " + this.identifierValue() + " -- It is read from " + isPreCompiledFromFile);			
    	} else if (CLASSFILE_ALREADY_GENERATED) {
			if(DocumentManager.verbose) Util.println("Skip  buildClassFile: " + this.identifierValue() + " -- It is already generated");			
    	} else {
...
//    		Util.dumpStack();
    ...
    protected void buildAndLoadOrAddClassFile(final SimulaCoder simCoder) throws IOException {
		if (this.isPreCompiledFromFile != null) {
			if(DocumentManager.verbose) Util.println("Skip  buildClassFile: "+this.identifierValue());
		} else {
    ...
    protected byte[] getBytesFromFile() {
    	Util.println("DeclarationScope.getBytesFromFile: ");
    ...
    protected void writeClassFile(final SimulaCoder simCoder, final byte[] bytes) throws IOException {
    	if(bytes != null) {
    		String entryName = DocumentManager.packetName + "/" + externalIdent + ".class";
    		String fileName = simCoder.tempClassFileDir.toString() + '/' + entryName;
//    		Util.println("DeclarationScope.writeClassFile: " + entryName);
//    		Util.println("DeclarationScope.writeClassFile: " + fileName);
	...
	private void listGeneratedClassFile(final SimulaCoder simCoder, final byte[] bytes) throws IOException {
        File outputFile = new File(simCoder.tempClassFileDir + "\\" + DocumentManager.packetName + "\\" + externalIdent + ".class");
        outputFile.getParentFile().mkdirs();
        try (FileOutputStream oupt = new FileOutputStream(outputFile)) {
			oupt.write(bytes); //oupt.flush(); oupt.close();
		}
        if(DocumentManager.verbose) Util.println("ClassFile written to: " + outputFile + "  nBytes="+bytes.length);
