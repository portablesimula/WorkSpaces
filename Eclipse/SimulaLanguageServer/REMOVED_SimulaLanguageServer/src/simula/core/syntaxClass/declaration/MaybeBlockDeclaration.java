...
//	}
	...
	public BlockStatement expectMaybeBlock(SimulaBuilder simBuilder) {
		...
		if(Option.TRACE_ACCEPT_STATEMENT > 0) Util.println("BlockStatement expectMaybeBlock: BEGIN " + debugName);

		simBuilder.consume(KeyWord.BEGIN); // consume BEGIN (add it to tokenList)

		parseBlock(simBuilder);

//		Util.println("MaybeBlockDeclaration.expectMaybeBlock: GOT END or EOF");
		...
		if(Option.TRACE_ACCEPT_STATEMENT > 0) Util.println("MaybeBlockDeclaration.expectMaybeBlock: ENDOF " + debugName + "  " + block);
...
//		ASSERT_SEMANTICS_CHECKED(); // TODO: ER DETTE BRA ?
	...
	public byte[] buildClassFile(final SimulaCoder simCoder) {
		...
		if(DocumentManager.verbose) Util.println("SubBlock.buildClassFile: "+CD_ThisClass); 
		...
		for (Statement stm : statements) {
			if(!(stm instanceof DummyStatement)) {
				...
				if(stm.firstLineNumber() < 0) { // TODO: TESTING
					Util.println("MaybeBlockDeclaration.build_STMS: stm="+stm.getClass().getSimpleName()+" "+stm+" LINE:"+stm.firstLineNumber());
					if(stm instanceof BlockStatement bst) {
						Util.println("MaybeBlockDeclaration.build_STMS: bst="+bst.getClass().getSimpleName()+" "+bst+" LINE:"+bst.firstLineNumber());
					}
				}
	...
	public void printTree(final int indent) {
		...
		Util.println(edTreeIndent(indent) + block + " " + identifierValue() + tail + "  declaredIn="+this.declaredIn);
