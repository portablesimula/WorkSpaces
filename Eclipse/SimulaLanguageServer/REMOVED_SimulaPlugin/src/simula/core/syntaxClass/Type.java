...
import simula.core.syntaxClass.expression.MissingExpression;
...
//		Util.println("NEW Type: " + classIdent);
	}
...
//				Util.println("Type.doChecking: refIdent="+refIdent.value+", qual="+qual);
			}
		}
	...
	public static boolean checkType(Expression expr, int keyWord) {
		...
		if(expr instanceof VariableExpression var) {
			Util.println("ConditionalExpression.doChecking: var.meaning: " + var.meaning);
	...
	public boolean isSubReferenceOf(final Type other) {
		...
		if(otherRef==null) result=false;     // No ref is a super-reference of NONE
			...
			ClassDeclaration thisDecl=(ClassDeclaration)CoreGlobal.getCurrentScope().findMeaning(thisRef).declaredAs;
			ClassDeclaration otherDecl=(ClassDeclaration)CoreGlobal.getCurrentScope().findMeaning(otherRef).declaredAs;
