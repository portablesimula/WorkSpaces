...
//		Util.println("NEW ConnectionStatement: add inspectVariableDeclaration to "+scope);
	...
	public void doJavaCoding(final SimulaCoder simCoder) {
		CoreGlobal.sourceLineNumber = firstLineNumber();
//		Util.println("ConnectionStatement.doJavaCoding: "+Global.sourceLineNumber);
	...
	public void printTree(final int indent) {
		Util.println(edTreeIndent(indent)+"INSPECT " + inspectedVariable + " = " + objectExpression);
		for (ConnectionDoPart doPart : connectionPart) doPart.printTree(indent + 1);
		if(otherwise != null) {
			Util.println(edTreeIndent(indent)+"OTHERWISE");
