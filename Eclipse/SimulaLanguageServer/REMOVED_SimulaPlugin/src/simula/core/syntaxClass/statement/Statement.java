...
//		Util.println("Statement.acceptStatement: LabeledStatement: current=" + Parse.getCurrentParserToken(simBuilder));//	+ ", prev=" + PsiParse.prevToken);
...
//			Util.println("\n\nStatement.acceptStatement: MAYBE LABEL IDENTIFIER: " + mayBeLabel);
		...
		if (labels != null) {
//			Util.println("Statement.acceptStatement: LABELS: " + labels);
//			Util.println("Statement.acceptStatement: prevParserToken: " + simBuilder.prevParserToken());
			statement = new LabeledStatement(simBuilder.documentManager, labels, statement);
//			Util.println("Statement.acceptStatement: DONE LabeledStatement: " + statement);				
		}
//		Util.println("Statement.acceptStatement: DONE: " + statement);
		return (statement);
	}
	...
	private static Statement acceptUnlabeledStatement(SimulaBuilder simBuilder) {
		...
		if(Option.TRACE_ACCEPT_STATEMENT > 1) Util.println("\nStatement.acceptUnlabeledStatement: "+simToken);
		Statement statement = null;
...
//				}
				...
				if(expr!=null) {
					if(expr instanceof VariableExpression var) {
						if(Option.TRACE_ACCEPT_STATEMENT > 1) {
							Util.println("Statement.acceptUnlabeledStatement: GOT VariableExpression: "+var);
						}
...
//							Util.println("Statement.acceptUnlabeledStatement: GOT BlockStatement: "+statement);
							break;
		      			} else {
		      				statement = new StandaloneExpression(simBuilder.documentManager, expr);
//							Util.println("Statement.acceptUnlabeledStatement: GOT StandaloneExpression(1): "+statement);
		      				break;
	      				}
	      			} else {
	      				statement = new StandaloneExpression(simBuilder.documentManager, expr);
//						Util.println("Statement.acceptUnlabeledStatement: GOT StandaloneExpression(2): "+statement);
	      				break;
	      			}
