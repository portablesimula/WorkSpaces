...
//			Util.syntaxError(simBuilder.getCurrentParserToken(), "parseExternalDeclaration: Expecting CLASS or PROCEDURE");
		...
		LOOP: while (true) {
//			Util.println("ExternalDeclaration.expectExternalDeclaration: identifier=" + identifierValue());
...
//				Util.println("ExternalDeclaration.expectExternalDeclaration: " + token.getClass().getSimpleName());
...
//			Util.println("ExternalDeclaration.expectExternalDeclaration: externalDeclaration: " + externalDeclaration);

			File jarFile = JarFileBuilder.findJarFile(simBuilder, identifier.value, externalIdentifier);
//			Util.println("ExternalDeclaration.expectExternalDeclaration: jarFile: " + jarFile);
			if(jarFile == null) {
	...
	public void doChecking() {
		...
		Util.println("ExternalDeclaration.doChecking: +++++++++++++++++ BASICIO ++++++++++++++++++++++");
//		StandardClass.BASICIO.printTree(0, decl);
		for(Declaration memb:StandardClass.BASICIO.declarationList) {
			Util.println("ExternalDeclaration.doChecking: " + memb);
		}
		Util.println("ExternalDeclaration.doChecking: +++++++++++++++++ CURRENT SCOPE "+CoreGlobal.getCurrentScope().identifierValue() +" ++++++++++++++++++++++");
		for(Declaration z:CoreGlobal.getCurrentScope().declarationList) {
			Util.println("ExternalDeclaration.doChecking: " + z);
		...
		Util.println("ExternalDeclaration.doChecking: " + this);
