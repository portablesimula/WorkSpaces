...
//		Diagnostic diagnostic = new Diagnostic(DiagnosticSeverity.Error, elt.lexRange, msg);
	...
	public static void doListDirectory(final String title, final String directoryPath) {
        ...
        Util.println("================== LIST DIRECTORY: " + title + ", Path: " + directoryPath + " ======================");
        if (!Files.exists(path)) { Util.println("Error: The directory does not exist: " + directoryPath); return; }
        if (!Files.isDirectory(path)) { Util.println("Error: The specified path is a file, not a directory: " + directoryPath); return; }
        // Try-with-resources ensures that the stream is closed automatically
        try (Stream<Path> stream = Files.list(path)) {
            if (!stream.iterator().hasNext()) { Util.println("Info: The directory is empty."); return; }
            try (Stream<Path> listStream = Files.list(path)) {
//                listStream.forEach(p -> Util.println("- " + p.getFileName()));
	...
	public static void doListClassFile(final String classFileName) {
		...
		Util.println("******** ENDOF List ClassFile: "+classFileName + " *****************************************************\n\n");
	}
...
//			Util.println("Util.execute: " + line);
...
//	}
//	
//	private boolean checkInLanguageServer() {
//		
//	}
