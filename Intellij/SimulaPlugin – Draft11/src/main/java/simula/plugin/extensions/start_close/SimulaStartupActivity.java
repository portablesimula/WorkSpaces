...
//    }
    ...
    public static void run(@NotNull Project project) {
        ...
        System.out.println("+++++++++++++++++++++++++++++++++++++++++++++++");
        System.out.println("A new project was opened: " + project.getName());
...
//       projectView.changeView(ScopeViewPane.ID, ProjectFilesScope.INSTANCE.getName());
    ...
    private static void initProject(Project project) {
        ...
        System.out.println("SimulaStartupActivity.initProject: project=" + project);
//        listAllModules(project);
        VFS.test(project);
        VirtualFile baseDir = project.getBaseDir();
        VFS.createDirectory(baseDir, "src");
        VFS.createDirectory(baseDir, "bin");
        VFS.createDirectory(baseDir, "ssf");
//        ApplicationManager.getApplication().runWriteIntentReadAction(() -> {
////        ApplicationManager.getApplication().runWriteAction(() -> {
//                        try {
//                            System.out.println("SimulaStartupActivity.initProject: project.getBaseDir: " + project.getBaseDir());
//                            VfsUtil.createDirectoryIfMissing(project.getBaseDir(), "smp");
//                            VfsUtil.createDirectoryIfMissing(project.getBaseDir(), "src");
//                            VfsUtil.createDirectoryIfMissing(project.getBaseDir(), "bin");
//                        } catch (IOException e) {
...
//        PluginModuleConfigurator.addSourceDirectory(module, srcDir);

    }

    public void createVfsFileExample(VirtualFile parentDirectory, String fileName, byte[] content) {
        // VFS modifications must be run within a write action
        ApplicationManager.getApplication().runWriteAction(() -> {
            try {
                // 1. Create a new child VirtualFile instance
                ...
                // The 'this' argument is a requestor object, often an action or component instance.
    ...
    public void listAllModules(Project project) {
        ModuleManager moduleManager = ModuleManager.getInstance(project);
        Module[] modules = moduleManager.getModules();

        for (Module module : modules) {
            Util.TRACE("Found module: " + module.getName());
        }
    }
