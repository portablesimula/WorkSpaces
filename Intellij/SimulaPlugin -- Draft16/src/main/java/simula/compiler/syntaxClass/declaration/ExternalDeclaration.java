...
/// "https://github.com/portablesimula/WorkSpaces/Eclipse/blob/main/SimulaCompiler2/Simula/src/simula/compiler/syntaxClass/declaration/ExternalDeclaration.java">
/// <b>Source File</b></a>.
/// 
...
public final class ExternalDeclaration extends Declaration {
	...
	private ExternalDeclaration(String identifier,String extIdentitier) {
		super("ExternalDeclaration", identifier);
	...
	private ExternalDeclaration() {
		super("ExternalDeclaration", null);
		this.declarationKind = ObjectKind.ExternalDeclaration;
	}
	...
	public static Vector<ExternalDeclaration> expectExternalHead(final PsiBuilder simBuilder, final BlockDeclaration enclosure) {
		...
		String identifier = Parse.expectIdentifier(simBuilder);
		LOOP: while (true) {
			Identifier externalIdentifier = null;
			if (Parse.accept(simBuilder, KeyWord.EQ)) {
				externalIdentifier = (Identifier) Parse.currentToken(simBuilder);
